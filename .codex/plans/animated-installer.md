# Comprehensive Codex-Mem Installer with @clack/prompts

## Overview

Build a beautiful, animated CLI installer for codex-mem using `@clack/prompts` (v1.0.1). Distributable via `npx codex-mem-installer` and `curl -fsSL https://install.cmem.ai | bash`. Replaces the need for users to manually clone, build, configure settings, and start the worker.

**Worktree**: `feat/animated-installer` at `.codex/worktrees/animated-installer`
...
- Plugin registration: `plugin/.codex-legacy-plugin/plugin.json`, `.codex-legacy-plugin/marketplace.json`
...
1. **Create directory structure** in the worktree:
   ...
   │       └── settings-writer.ts # Write ~/.codex-mem/settings.json
...
2. **Create `package.json`**:
   ```json
   {
     "name": "codex-mem-installer",
     "version": "1.0.0",
     "type": "module",
     "bin": { "codex-mem-installer": "./dist/index.js" },
     "files": ["dist"],
     "scripts": {
...
1. **`src/index.ts`** — Entry point:
   - TTY guard: if `!process.stdin.isTTY`, print error directing user to `npx codex-mem-installer`, exit 1
   - Import and call `runInstaller()` from steps
   - Top-level catch → `p.cancel()` + exit 1

2. **`src/steps/welcome.ts`** — Welcome step:
   - `p.intro()` with styled title using picocolors: `" codex-mem installer "`
   - Display version info via `p.log.info()`
   - Check if already installed (detect `~/.codex-mem/settings.json` and `~/.codex/plugins/marketplaces/thedotmack/`)
   - If upgrade detected, `p.confirm()`: "codex-mem is already installed. Upgrade?"
...
1. **`src/steps/ide-selection.ts`** — IDE picker:
   - `p.multiselect()` with options:
     - Codex Code (default selected, hint: "recommended")
     - Cursor
     - Windsurf (hint: "coming soon", disabled: true)
   - For Codex Code: explain plugin will be registered via marketplace
...
2. **`src/steps/provider.ts`** — AI provider configuration:
   - `p.select()` for provider:
     - **Codex** (hint: "recommended — uses your Codex subscription")
...
1. **`src/steps/settings.ts`** — Settings wizard:
   ...
   - If customizing, use `p.group()` for:
     ...
     - **Data directory**: `p.text()` with default `~/.codex-mem`
...
2. **`src/utils/settings-writer.ts`** — Write settings:
   ...
   - Write to `~/.codex-mem/settings.json`
   - Create `~/.codex-mem/` directory if it doesn't exist
...
1. **`src/steps/install.ts`** — Installation runner:
   - Use `p.tasks()` for visual progress:
     - **"Cloning codex-mem repository"**: `git clone --depth 1 https://github.com/thedotmack/codex-mem.git` to temp dir
     - **"Installing dependencies"**: `npm install` in cloned repo
     - **"Building plugin"**: `npm run build` in cloned repo
     - **"Registering plugin"**: Copy plugin files to `~/.codex/plugins/marketplaces/thedotmack/`
       - Create marketplace.json, plugin.json structure
       - Register in `~/.codex/plugins/known_marketplaces.json`
       - Add to `~/.codex/plugins/installed_plugins.json`
       - Enable in `~/.codex/settings.json` under `enabledPlugins`
...
2. **`src/steps/worker.ts`** — Worker startup:
   - Use `p.spinner()` for worker startup:
     ...
     - Write PID file to `~/.codex-mem/worker.pid`
...
### Verification
- [ ] Plugin files exist at `~/.codex/plugins/marketplaces/thedotmack/`
...
1. **`src/steps/complete.ts`** — Completion screen:
   ...
   - `p.note()` with next steps:
     - "Open Codex Code and start a conversation — memory is automatic!"
     - "View your memories: http://localhost:{port}"
     - "Search past work: use /mem-search in Codex Code"
...
- [ ] Plugin appears in Codex Code plugin list
