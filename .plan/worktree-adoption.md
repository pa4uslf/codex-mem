...
| Chroma collection architecture | `src/services/sync/ChromaSync.ts` | 806 (comment) | **Single shared collection `cm__codex-mem`**, scoped by metadata. Do NOT create a per-merged collection. |
...
- Do NOT create a new Chroma collection for merged observations. Deployment uses a single shared `cm__codex-mem` collection.
...
- `sqlite3 ~/.codex-mem/codex-mem.db ".schema observations"` shows `merged_into_project TEXT`.
- Same for `session_summaries`.
- Restart worker → no ALTER TABLE error (guard worked).
- `sqlite3 ~/.codex-mem/codex-mem.db ".indices observations"` lists `idx_observations_merged_into`.
...
- Chroma: `chroma_get_documents` with `where: { merged_into_project: 'codex-mem' }` returns the same row count.
...
- Before adoption: context-inject API for `codex-mem` returns N observations.
- After adoption of `codex-mem/dar-es-salaam`: API returns N + M (M = count of dar-es-salaam's own observations).
- Semantic search via Chroma (`/search` endpoint or MCP) with `project=codex-mem` returns dar-es-salaam-origin rows too.
- Worktree-local queries (`projects=[codex-mem, codex-mem/dar-es-salaam]`) still return `[parent + own]` unchanged.
...
**What to implement**: `codex-mem adopt [--branch <name>] [--dry-run]` — covers squash-merge where `git branch --merged` returns nothing, and provides a manual override for any adoption run.
...
- `npx codex-mem adopt --dry-run` in a repo with merged worktrees prints what WOULD be adopted without writing.
- `npx codex-mem adopt` writes + prints counts.
- `npx codex-mem adopt --branch feature/foo` forces adoption of that branch even if `git branch --merged` doesn't include it (squash case).
...
- Start worker → create synthetic observations under `codex-mem/test-wt` → simulate branch merge (`git merge`) → restart worker → context-inject API for `codex-mem` returns test-wt observations.
- Same flow with a squash-merge → auto-adoption misses → run `codex-mem adopt --branch test-wt` → API now returns them.
- Re-run `codex-mem adopt` twice: second run reports `adoptedObservations: 0, chromaUpdates: 0`.
