...
Email Investigation Batch Processor using Codex-mem's email-investigation mode.

## Overview

Ragtime processes email corpus files through Codex, using the email-investigation mode for entity/relationship/timeline extraction. Each file gets a NEW session - context is managed by Codex-mem's context injection hook, not by conversation continuation.
...
| `RAGTIME_PLUGIN_PATH` | `./plugin` | Path to codex-mem plugin |
| `CODEX_MEM_WORKER_PORT` | `37777` | Worker service port |
...
1. **Startup**: Sets `CODEX_MEM_MODE=email-investigation` and cleans up old transcripts
2. **Processing**: For each file:
   - Starts a NEW Codex session (no continuation)
   - Codex reads the file and analyzes entities, relationships, timeline events
   - Codex-mem's context injection hook provides relevant past observations
...
The main codex-mem repository is licensed under Apache-2.0, and ragtime now uses the same license for this migration.
