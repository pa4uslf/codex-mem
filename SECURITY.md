...
Only the latest released version of `codex-mem` receives security updates. Please upgrade to the latest version before reporting a vulnerability.
...
If you discover a security vulnerability in codex-mem, please report it by:
...
**Scope:** This policy covers the `codex-mem` plugin and its bundled components (hooks, worker service, SQLite/Chroma sync, viewer UI, search/planning skills). Issues in upstream dependencies should be reported to those projects directly, but feel free to flag them to us as well.
...
Codex-mem executes system commands for git operations and process management. We have implemented comprehensive protections against command injection:
...
- **PID File Protection:** Process IDs are stored in user's data directory (`~/.codex-mem/`)
...
Codex-mem includes dual-tag system for content privacy:
...
Codex-mem stores data locally in `~/.codex-mem/`:

- **Database:** SQLite3 at `~/.codex-mem/codex-mem.db`
- **Vector Store:** Chroma at `~/.codex-mem/chroma/`
- **Logs:** `~/.codex-mem/logs/`
- **Settings:** `~/.codex-mem/settings.json`

All codex-mem state files (database, vector store, logs, settings, supervisor and PID files) are written to the local user directory and are not uploaded by codex-mem itself. Codex-mem does not collect telemetry.

However, by design codex-mem invokes upstream model providers and optional integrations to do its work, so observation/transcript/prompt content can leave the machine through those channels:

- **Codex Agent SDK** (default summarization/observation path): sends prompts and transcript context to Codex's API.
...
- **OAuth / keychain reads**: codex-mem reads the Codex Code OAuth token from the platform-native credential store at spawn time. The token is injected into worker subprocesses but is not transmitted by codex-mem.
- **GitHub releases / npm registry**: version-check and self-update flows fetch metadata from public registries.

Review your provider/Chroma configuration in `~/.codex-mem/settings.json` and `~/.codex-mem/.env` before sending sensitive content. Use `<private>...</private>` tags to keep specific content out of the local store.

## Permissions

Codex-mem requires:

- **File System:** Read/write to `~/.codex-mem/` and `~/.codex/plugins/`
...
1. Keep codex-mem updated to the latest version
