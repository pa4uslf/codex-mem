...
Codex is still exposing `codex-mem` from:

```text
/Users/alexnewman/.codex/plugins/cache/thedotmack/codex-mem/12.3.1
```

That cache entry is the source of the `codex-mem:...` skills loaded in this Codex session. The working tree and the Codex marketplace clone both advertise `12.7.2`, but the enabled Codex plugin points at the old installed cache. This is not a model-memory issue.
...
- Codex marketplace source is current:
  - `/Users/alexnewman/.codex/config.toml` contains `[marketplaces.codex-mem-local]`
  - `last_updated = "2026-05-06T23:13:59Z"`
  - `last_revision = "bb3dbfdb5ae92b55b7e4686e4904995184261232"`
  - `/Users/alexnewman/.codex/.tmp/marketplaces/codex-mem-local/package.json` is `12.7.2`
  - `/Users/alexnewman/.codex/.tmp/marketplaces/codex-mem-local/.codex-plugin/plugin.json` is `12.7.2`

- Active enabled plugin state is still old:
  - `/Users/alexnewman/.codex/config.toml` contains `[plugins."codex-mem@thedotmack"] enabled = true`
  - The only `codex-mem` plugin cache under `~/.codex/plugins/cache/thedotmack/codex-mem` is `12.3.1`
  - `/Users/alexnewman/.codex/plugins/cache/thedotmack/codex-mem/12.3.1/package.json` is `12.3.1`
  - `/Users/alexnewman/.codex/plugins/cache/thedotmack/codex-mem/12.3.1/.install-version` records `{"version":"12.3.1", ...}`

- The active cache is not shaped like the new first-class Codex bundle:
  - It has `.codex-legacy-plugin/plugin.json`
  - It does not have `.codex-plugin/plugin.json`
  - It does not have `hooks/codex-hooks.json`
  - Its `.mcp.json` still uses the old `bun` command with `"${CODEX_PLUGIN_ROOT}/scripts/mcp-server.cjs"`
...
1. Marketplace source state: current and registered as `codex-mem-local`.
2. Installed plugin cache state: stale and enabled as `codex-mem@thedotmack`.

Codex loads skills, hooks, and MCP metadata from the installed plugin cache, not directly from the marketplace source. Since the installed cache is still `12.3.1`, every new Codex session sees `codex-mem` as `12.3.1`, even though the marketplace clone is already at `12.7.2`.

The `codex-mem@thedotmack` plugin ID also suggests this cache came from an older GitHub marketplace install path, while the current installer registers `codex-mem-local`. That mismatch needs to be handled explicitly during repair and install.
...
- Capture a clean before-state snapshot:
  ...
  - `find ~/.codex/plugins/cache/thedotmack/codex-mem -maxdepth 2 -type d -print`

- Confirm which paths Codex injects into the session skill list:
  - Start a fresh Codex session.
  - Inspect the available skills list for `codex-mem:` paths.
  - Expected current bad path: `~/.codex/plugins/cache/thedotmack/codex-mem/12.3.1/skills`.
...
- Back up the current Codex plugin state:
  ...
  - Archive or copy `~/.codex/plugins/cache/thedotmack/codex-mem/12.3.1`

- Remove the stale enabled plugin state through supported UI where possible:
  - Open Codex.
  - Run `/plugins`.
  - Disable or uninstall `codex-mem@thedotmack` if it appears.

- Register or refresh the current marketplace:
  - `codex plugin marketplace upgrade codex-mem-local`
  - If needed, re-add from the durable local marketplace root produced by the installer.

- Install `codex-mem` from the `codex-mem (local)` marketplace in `/plugins`.

- Restart Codex.

Verification:

- `~/.codex/plugins/cache` contains a `codex-mem` cache with `.codex-plugin/plugin.json`.
...
- A fresh Codex session lists `codex-mem:` skills from the new cache, not `12.3.1`.

Anti-pattern guards:

- Do not manually copy the repository into `~/.codex/plugins/cache` as the primary fix. Use it only as a diagnostic fallback.
- Do not leave both `codex-mem@thedotmack` and a new local `codex-mem` enabled if Codex treats them as distinct plugins.
...
- Add a post-registration diagnostic that checks whether an enabled stale `codex-mem` plugin is already present.
...
- Add a `diagnoseCodexPluginState()` helper that reads:
  - `~/.codex/config.toml`
  - `~/.codex/plugins/cache/**/codex-mem/**/.codex-plugin/plugin.json`
  - `~/.codex/plugins/cache/**/codex-mem/**/.codex-legacy-plugin/plugin.json`
  - `~/.codex/plugins/cache/**/codex-mem/**/.install-version`

- Classify state as:
  - `not_installed`
  - `installed_current_codex`
  - `installed_stale_codex`
  - `installed_legacy_codex_shape`
  - `duplicate_installs`

- Include current repo/package version in the expected state.
- Treat `installed_legacy_codex_shape` as a warning or failure for Codex integration, because it is the exact observed bad state.

Verification:

- Unit tests cover stale `12.3.1` legacy cache with `.codex-legacy-plugin` only.
...
- Extend `npx codex-mem repair --ide codex-cli` or equivalent repair flow to handle Codex first-class plugin state.
- The repair should:
  - Register or upgrade the local marketplace.
  - Detect stale enabled `codex-mem@thedotmack`.
...
- Repair does not remove unrelated Codex settings or non-codex-mem plugins.
...
- Document that Codex currently has two steps:
  - Marketplace registration via `npx codex-mem install`.
  - Plugin install/enable via `/plugins`.

- Add troubleshooting for this exact mismatch:
  - Symptom: Codex skill list shows `~/.codex/plugins/cache/thedotmack/codex-mem/12.3.1`.
  - Cause: stale installed plugin cache, despite current marketplace source.
  - Fix: uninstall old `codex-mem@thedotmack`, install from `codex-mem (local)`, restart Codex.
...
- Duplicate install case with both `codex-mem@thedotmack` and local `codex-mem`.

Acceptance criteria:

- Fresh Codex session loads `codex-mem:` skills from a current cache path.
...
- Does `codex plugin marketplace upgrade codex-mem-local` intentionally avoid updating already-installed plugin caches?
- Is there a hidden or upcoming non-interactive plugin install command that can replace the manual `/plugins` step?
- Should the installer remove or disable `codex-mem@thedotmack` when installing `codex-mem-local`, or should it only warn?
