...
> **Scope**: Fix accumulated worker / daemon lifecycle bugs in codex-mem.
...
| File | Why |
| --- | --- |
| `CODEX.md` (project root) | Architecture, exit-code strategy, Pro/OSS boundary, settings conventions |
...
| `src/shared/worker-utils.ts` | `recordWorkerUnreachable` (line 401), `executeWithWorkerFallback` (line 443), fail-loud counter file at `~/.codex-mem/state/hook-failures.json` |
...
**Settings** — `SettingsDefaultsManager.get('CODEX_MEM_…')`, `SettingsDefaultsManager.loadFromFile(path)`. New keys require: (a) type added to the interface in `SettingsDefaultsManager.ts`, (b) default value declared in the same file, (c) documented in CODEX.md if user-tunable.
...
- **Do not** call `process.exit(1)` on hook-side error paths — it accumulates Windows Terminal tabs (CODEX.md exit-code strategy). Use `0` for graceful, `2` only for blocking-error paths that need to surface stderr to Codex.
...
3. **Catalog every retry / unreachable site**:
   ...
   - `src/utils/codex-md-utils.ts:305` — direct `workerHttpRequest`
...
4. **Catalog every spawn site**:
   ...
   - `spawnSdkProcess` (`src/supervisor/process-registry.ts:532`) — Codex SDK subprocesses
...
- Confirmed list of which `executeWithWorkerFallback` callers run inside hooks (Codex Code's strict timeout window) vs. inside the worker (no timeout pressure) — this drives Phase 4 circuit-breaker scoping.
...
5. **Diagnostic stderr**: when port-in-use without our worker responding, write to stderr (and log INFO) with: `codex-mem worker port <N> in use by an unidentified process; not spawning duplicate`. This must NOT block the hook — exit 0 still per CODEX.md.

### 5.3 New settings

| Key | Default | Range | Purpose |
| --- | --- | --- | --- |
| `CODEX_MEM_DAEMON_LOCK_TIMEOUT_MS` | `5000` | 0–60000 | Max wait for the spawn lock |
| `CODEX_MEM_PID_PORT_RECHECK_MS` | `2000` | 500–30000 | Wait window before treating port-in-use without `/api/version` response as "unknown listener" |

### 5.4 Acceptance criteria

- Run two `codex-mem start` commands in parallel → exactly one daemon ends up alive; the other exits cleanly with a log line referencing the lock.
...
| `src/services/maintenance/DbMaintenance.ts` | Startup check: if `freelist_count / page_count > FREE_RATIO_VACUUM_THRESHOLD` (default 0.40), perform full `VACUUM` after `VACUUM INTO` backup to `<DATA_DIR>/backups/codex-mem-pre-vacuum-<ts>.db`. Pause queue processor first. |
...
2. **Periodic incremental vacuum + WAL checkpoint**:
   - Schedule via `setInterval` with `.unref()`. Default cadence: 24h. Setting: `CODEX_MEM_DB_MAINTENANCE_INTERVAL_HOURS` (default `24`, min `1`, max `168`).
...
3. **Startup full VACUUM (one-shot per session) when free-ratio is high**:
   ...
   - If `freelist_count / page_count >= CODEX_MEM_DB_VACUUM_THRESHOLD_RATIO` (default `0.40`), schedule a deferred VACUUM (5 minutes after worker becomes ready) to avoid slowing startup.
...
5. **Cleanup-V12.4.3 regression detection**: Re-scan `sdk_sessions WHERE project = OBSERVER_SESSIONS_PROJECT` and `pending_messages` matching the stuck-pending pattern at maintenance ticks. If any match AND the marker exists, log `MAINTENANCE` WARN and re-run the purge (idempotent). Setting: `CODEX_MEM_CLEANUP_REGRESSION_CHECK = true`.

### 6.3 New settings

| Key | Default | Range | Purpose |
| --- | --- | --- | --- |
| `CODEX_MEM_DB_MAINTENANCE_ENABLED` | `true` | bool | Master kill-switch |
| `CODEX_MEM_DB_MAINTENANCE_INTERVAL_HOURS` | `24` | 1–168 | Periodic cadence |
| `CODEX_MEM_DB_VACUUM_THRESHOLD_RATIO` | `0.40` | 0.05–0.95 | Free-ratio above which we auto-VACUUM at startup |
| `CODEX_MEM_DB_VACUUM_STARTUP_DELAY_MS` | `300000` (5 min) | 0–3600000 | Defer startup VACUUM so it doesn't block readiness |
| `CODEX_MEM_CLEANUP_REGRESSION_CHECK` | `true` | bool | Re-scan v12.4.3-shaped pollution |
...
Per tick (default 30s, gated by `CODEX_MEM_REAPER_ENABLED`):
...
5. **Hard delete is opt-in** via `CODEX_MEM_REAPER_HARD_DELETE_INACTIVE_DAYS` (default `0` = disabled; nonzero = days threshold). When enabled and a session has `inactive_at` older than the threshold AND no FK-referencing rows, hard-delete the session row. Default-off because user data safety > disk space.

### 2.3 New settings

| Key | Default | Range | Purpose |
| --- | --- | --- | --- |
| `CODEX_MEM_REAPER_ENABLED` | `true` | bool | Master switch |
| `CODEX_MEM_REAPER_TICK_MS` | `30000` | 5000–600000 | Tick cadence (piggy-backs supervisor; this value gates whether the reaper runs each tick) |
| `CODEX_MEM_REAPER_PROCESSING_STUCK_MS` | `300000` (5 min) | 30000–86400000 | Threshold for a `processing` row to be considered stuck |
| `CODEX_MEM_REAPER_INACTIVE_DAYS` | `30` | 1–365 | When to mark a session `inactive_at` |
| `CODEX_MEM_REAPER_HARD_DELETE_INACTIVE_DAYS` | `0` | 0–365 | 0 = never; otherwise, hard-delete inactive rows older than N days |
...
| Key | Default | Range | Purpose |
| --- | --- | --- | --- |
| `CODEX_MEM_CHROMA_IDLE_SHUTDOWN_MS` | `900000` (15 min) | 60000–86400000 | Idle reaper threshold |
| `CODEX_MEM_CHROMA_ORPHAN_SCAN_ON_START` | `true` | bool | Master switch for startup scan |
| `CODEX_MEM_CHROMA_MAX_CONCURRENT` | `1` | 1–4 | Cap chroma-mcp instances per worker |
...
- **Do not** spin up the idle-reaper timer if `chromaMcpManager` is null (chroma disabled via `CODEX_MEM_CHROMA_ENABLED=false`).
...
| File | Change |
| --- | --- |
| `src/shared/worker-circuit-breaker.ts` (new) | `CircuitBreaker` class: states `CLOSED`, `OPEN`, `HALF_OPEN`. Persist to `~/.codex-mem/state/circuit-breaker.json`. |
...
| --- | --- | --- |
| `CODEX_MEM_BREAKER_FAILURE_THRESHOLD` | `5` | 1–50 |
| `CODEX_MEM_BREAKER_RESET_TIMEOUT_MS` | `30000` | 1000–600000 |
| `CODEX_MEM_BREAKER_HALF_OPEN_MAX_PROBES` | `1` | 1–10 |
| `CODEX_MEM_BREAKER_LIFETIME_CAP` | `50` | 0–10000 (0 = no cap) |
...
5. **Lifetime cap**: when `lifetimeFailures > CODEX_MEM_BREAKER_LIFETIME_CAP`, transition to `OPEN_PERMANENT`. The only way out is the manual-reset API or restarting the worker with a fresh state file. Print prominent stderr: `codex-mem: 50 lifetime worker failures detected. Disabling memory hooks until reset. Run: codex-mem worker doctor`.

### 4.4 Acceptance criteria

- Kill the worker, run 100 hooks → exactly `CODEX_MEM_BREAKER_FAILURE_THRESHOLD` HTTP attempts made; rest short-circuit.
...
```json
{
  ...
  "db": {
    ...
    "sdk_sessions_by_project": { "codex-mem": 25, "...": 120 }
  },
  "child_processes": {
...
`GET /api/healthz?format=prom` → 200 `text/plain` with Prometheus text format. One metric per JSON leaf (e.g. `codex_mem_db_free_ratio_pct 0.16`).

`status` derivation:
- `unhealthy` if breaker is OPEN_PERMANENT, OR DB initialization failed, OR chroma-mcp pid count > `CODEX_MEM_CHROMA_MAX_CONCURRENT`.
...
5. **Prometheus format**: emit `# HELP` and `# TYPE` lines per metric. Use the same naming convention (`codex_mem_<group>_<name>`).
...
| File | Change |
| --- | --- |
| `src/cli/handlers/worker-doctor.ts` (new) | New CLI subcommand `codex-mem worker doctor` — fetches `/api/healthz`, formats it for terminals, includes recent reaper actions. |
...
- breaker open → `codex-mem worker reset-breaker`
- DB free ratio high → mention next vacuum window
- chroma orphans → `codex-mem worker reap-chroma`
...
3. **Update CODEX.md**: add a "Worker Maintenance" section with: settings reference table, the doctor command, a brief description of the reaper/breaker/vacuum behavior. Per CODEX.md "Important: No need to edit the changelog ever" — only edit CODEX.md, never CHANGELOG.
...
- `codex-mem worker doctor` prints a green-OK summary on a healthy worker.
- `codex-mem worker doctor --json` returns valid JSON pipeable to `jq`.
- Killing the worker → `codex-mem worker doctor` cleanly reports `Worker unreachable` instead of hanging.
- CODEX.md updates are limited to a new section; no churn elsewhere.

### 8.5 Verification checklist

- [ ] `codex-mem worker doctor` exits 0 on healthy state, 1 on unhealthy, 2 if worker unreachable (mirrors hook-exit-codes convention).
...
Run the worker for 24 hours under realistic Codex Code usage. After 24h:

| Metric | Pass criterion |
| --- | --- |
| `ps aux \| grep chroma-mcp \| wc -l` | ≤ 1 |
| `ps aux \| grep codex-mem \| wc -l` | ≤ a small constant (1-2) |
...
| Two parallel `codex-mem start` | Exactly one daemon survives; lock log line visible |
...
git diff main -- src/shared/SettingsDefaultsManager.ts | grep "CODEX_MEM_"
...
- `CODEX.md` adds: Worker Maintenance section (Phase 8.3).
- `docs/public/` (optional): user-facing explanation.
- No CHANGELOG edits (auto-generated per CODEX.md).
...
| --- | --- | --- | --- |
| `CODEX_MEM_DAEMON_LOCK_TIMEOUT_MS` | 5 | `5000` | 0–60000 |
| `CODEX_MEM_PID_PORT_RECHECK_MS` | 5 | `2000` | 500–30000 |
| `CODEX_MEM_DB_MAINTENANCE_ENABLED` | 6 | `true` | bool |
| `CODEX_MEM_DB_MAINTENANCE_INTERVAL_HOURS` | 6 | `24` | 1–168 |
| `CODEX_MEM_DB_VACUUM_THRESHOLD_RATIO` | 6 | `0.40` | 0.05–0.95 |
| `CODEX_MEM_DB_VACUUM_STARTUP_DELAY_MS` | 6 | `300000` | 0–3600000 |
| `CODEX_MEM_CLEANUP_REGRESSION_CHECK` | 6 | `true` | bool |
| `CODEX_MEM_REAPER_ENABLED` | 2 | `true` | bool |
| `CODEX_MEM_REAPER_TICK_MS` | 2 | `30000` | 5000–600000 |
| `CODEX_MEM_REAPER_PROCESSING_STUCK_MS` | 2 | `300000` | 30000–86400000 |
| `CODEX_MEM_REAPER_INACTIVE_DAYS` | 2 | `30` | 1–365 |
| `CODEX_MEM_REAPER_HARD_DELETE_INACTIVE_DAYS` | 2 | `0` | 0–365 |
| `CODEX_MEM_CHROMA_IDLE_SHUTDOWN_MS` | 3 | `900000` | 60000–86400000 |
| `CODEX_MEM_CHROMA_ORPHAN_SCAN_ON_START` | 3 | `true` | bool |
| `CODEX_MEM_CHROMA_MAX_CONCURRENT` | 3 | `1` | 1–4 |
| `CODEX_MEM_BREAKER_FAILURE_THRESHOLD` | 4 | `5` | 1–50 |
| `CODEX_MEM_BREAKER_RESET_TIMEOUT_MS` | 4 | `30000` | 1000–600000 |
| `CODEX_MEM_BREAKER_HALF_OPEN_MAX_PROBES` | 4 | `1` | 1–10 |
| `CODEX_MEM_BREAKER_LIFETIME_CAP` | 4 | `50` | 0–10000 |
...
| `CODEX.md` | 8 (Worker Maintenance section) |
...
4. **Pro-features compatibility**: confirm with maintainers that `/api/healthz` does not duplicate any planned Pro endpoint. Per CODEX.md "Pro Features Architecture", the worker's local HTTP API stays open — `/api/healthz` is fine to add OSS-side.
