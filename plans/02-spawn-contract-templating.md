...
- Pro-feature endpoints or worker port resolution (uses `CODEX_MEM_WORKER_PORT`, not `CODEX_PLUGIN_ROOT`; orthogonal).
...
|---|---|---|---|---|
| 1 | `plugin/hooks/hooks.json` | 11, 24, 30, 42, 55, 68, 80 (every hook command) | Codex Code injects env var → bash expands `${CODEX_PLUGIN_ROOT:-${PLUGIN_ROOT:-}}` | 6 hook events. `shell: bash` set explicitly. |
...
| 5 | `plugin/scripts/version-check.js` | 7–17 | Reads `process.env.CODEX_PLUGIN_ROOT`, then falls back to `dirname(fileURLToPath(import.meta.url))/..`. | Runtime resolution layer. |
| 6 | `plugin/scripts/bun-runner.js` | 11 (`RESOLVED_PLUGIN_ROOT`), 13–21 (`fixBrokenScriptPath`), 168 (diagnostic emit) | Reads `process.env.CODEX_PLUGIN_ROOT`, falls back to script dirname. `fixBrokenScriptPath` is a band-aid: when arg starts with `/scripts/` (i.e., raw unsubstituted `${CODEX_PLUGIN_ROOT}/scripts/X.cjs` came through as `/scripts/X.cjs`), it prepends `RESOLVED_PLUGIN_ROOT`. | Runtime resolution layer. |
| 7 | `src/services/integrations/CodexCliInstaller.ts` | 60–78 (`resolvePluginMarketplaceRoot`), 66–67 (env vars consulted) | Reads `process.env.CODEX_PLUGIN_ROOT`, then `process.env.PLUGIN_ROOT`, then `process.cwd()`, then script dirname. | Install-time only. |
...
| 12 | `src/services/integrations/OpenCodeInstaller.ts` | 29–46 (`findBuiltPluginPath`) | NONE — copies `dist/opencode-plugin/index.js` to `~/.config/opencode/plugins/codex-mem.js`. | OpenCode runs JS in its own sandbox; no shell. |
| 13 | `src/integrations/opencode-plugin/index.ts` | 74–80 (`resolveWorkerPort`) | Uses `CODEX_MEM_WORKER_PORT` env (orthogonal to plugin-root scope). | No plugin-root templating. |
| 14 | `openclaw/install.sh` (1653 lines) | grep returns 0 hits for `CODEX_PLUGIN_ROOT` or `PLUGIN_ROOT`. Uses `${HOME}`, `${COLOR_*}`, etc. | N/A — OpenClaw configures via `configSchema` (`workerPort`, `workerHost`); no plugin-root templating. | Out of scope but documented for completeness. |
| 15 | `.codex-legacy-plugin/marketplace.json`, `.codex-legacy-plugin/plugin.json`, `.codex-plugin/plugin.json`, `plugin/.codex-legacy-plugin/plugin.json`, `plugin/.codex-plugin/plugin.json`, `.agents/plugins/marketplace.json` | manifest fields | NONE — relative paths only (`./plugin`, `./.mcp.json`, `./hooks/codex-hooks.json`). | Resolved by host marketplace machinery. |
| 16 | `docs/public/hooks-architecture.mdx` lines 100, 176, 223, 283, 337, 604, 754 | code examples | DOCS — currently teach users raw `${CODEX_PLUGIN_ROOT}/scripts/...` syntax. | These examples drive third-party copy-paste; must align with canonical rule chosen in Phase 1. |
...
| Site | Spawned by | `${CODEX_PLUGIN_ROOT}` substituted by | Shell semantics |
|---|---|---|---|
| `plugin/hooks/hooks.json` | Codex Code hook runner | Codex Code injects env; bash expands `${VAR:-default}` | bash (`shell: bash`) |
| `plugin/hooks/codex-hooks.json` | Codex CLI hook runner | Codex *should* inject env; sh expands | sh (no `shell` field) |
| `.mcp.json` / `plugin/.mcp.json` | Codex Code / Codex MCP loader | `sh -c "..."` expands `${VAR:-default}` | `sh -c` with args[] |
...
- Lines 110–114: every hook command must contain `CODEX_PLUGIN_ROOT`.
...
- Lines 84–99: MCP launcher includes `.codex/plugins/cache/codex-mem-local/codex-mem` and `plugins/cache/thedotmack/codex-mem` fallbacks; root and bundled launchers stay synced.
- Lines 135–177: full shell-prelude assertions for `.mcp.json`, codex hooks, and codex hooks (`${CODEX_CONFIG_DIR:-$HOME/.codex}`, `_E="${CODEX_PLUGIN_ROOT:-${PLUGIN_ROOT:-}}"`, `while IFS= read -r _R`, `[ -f "$_Q/scripts/..." ]`, `command -v cygpath`, etc.).

`tests/plugin-version-check.test.ts:10`: exercises `CODEX_PLUGIN_ROOT: root` env injection at version-check time.

### 0.4 Existing build-time enforcement

`scripts/build-hooks.js`:
- Lines 392–396: byte-identical sync between `.mcp.json` and `plugin/.mcp.json`.
- Lines 397–403: MCP launcher must include codex cache and codex cache fallbacks.
...
| Item | Location | Use |
|---|---|---|
| `CODEX_CONFIG_DIR` constant | `src/shared/paths.ts:41` | Used in shell template fallback as `${CODEX_CONFIG_DIR:-$HOME/.codex}` |
...
1. **Codex Code hook spec.** Does Codex Code documentation say `CODEX_PLUGIN_ROOT` is *guaranteed* to be set at hook spawn time? Or only when the hook is loaded from a plugin (vs. a user-level hook)? Source: https://docs.codex.com/codex-code/ — find the hook contract page.
...
7. **MCP server protocol.** Confirm that MCP server registration in IDE-owned `mcp.json` files (Cursor, Copilot, Antigravity, Goose, Roo, Warp) does not provide any `${VAR}` substitution — i.e., absolute paths are mandatory for those hosts. Source: Codex MCP docs.
...
- `${CODEX_PLUGIN_ROOT}` is **never expanded** at JSON-parse time. Any code that reads `.mcp.json` or `hooks.json` directly will see the literal string `${CODEX_PLUGIN_ROOT}` unless it shells out to bash/sh. Don't write tests that assume otherwise.
- Manifest files (`plugin.json`, `marketplace.json`) **do not** support `${VAR}` substitution per Codex/Codex marketplace specs. Don't propose adding it.

---

## Phase 1 — Codify the canonical resolution rule

**What to implement:** Decision document + amendment to `CODEX.md`. Code follows in Phases 2–4.

### 1.1 The three options (recap)

(a) **Always pre-resolve to absolute path at install time.** Every hook/MCP entry contains a hard-coded `/Users/<user>/.codex/plugins/cache/.../scripts/X.cjs`. Pro: zero spawn-contract surface. Con: every codex-mem version bump invalidates baked paths in IDE configs the host doesn't own (Cursor, Gemini, Windsurf, MCP-only IDEs, OpenClaw).

(b) **Always rely on POSIX-shell defensive expansion.** Hook/MCP entries contain `_E="${CODEX_PLUGIN_ROOT:-${PLUGIN_ROOT:-}}"; _P=$(... fallback chain ...)`. Pro: zero re-install needed across upgrades. Con: requires POSIX shell available to the host (Windows native cmd.exe doesn't qualify; cygpath workaround already addresses Git-Bash/MSYS).
...
- **Rule A (host-managed shell-template):** sites where the host (Codex Code, Codex CLI) owns the config file (`hooks.json`, `codex-hooks.json`, `.mcp.json`, `plugin/.mcp.json`) and may rotate the cache directory on plugin upgrade. Use the POSIX-shell defensive expansion (option b).
- **Rule B (installer-managed bake):** sites where codex-mem's installer owns the config file (Cursor, Gemini, Windsurf, MCP-only IDEs). Use the absolute-path bake (option a). On `codex-mem` version bump, the installer re-bakes paths idempotently.
- **Rule C (runtime resolution):** `plugin/scripts/version-check.js` and `plugin/scripts/bun-runner.js` accept BOTH `CODEX_PLUGIN_ROOT` env AND the script's own `dirname(import.meta.url)/..`, in that order. This is already the case (lines 7–17 of version-check.js, line 11 of bun-runner.js); document it.
...
Append to `CODEX.md` under a new `## Spawn-Contract Resolution` section (between `## Multi-account` and `## File Locations`):

```md
## Spawn-Contract Resolution

codex-mem integrations resolve `${CODEX_PLUGIN_ROOT}` (and equivalents) using one of three rules. Pick the rule by who owns the config file.

### Rule A — Host-managed shell-template (Codex Code, Codex CLI)

Sites: `plugin/hooks/hooks.json`, `plugin/hooks/codex-hooks.json`, `.mcp.json`, `plugin/.mcp.json`.

The host (Codex Code or Codex) owns the file's runtime location and rotates the cache directory on plugin upgrade. Hook/MCP `command` strings use the canonical defensive shell prelude:

    _C="${CODEX_CONFIG_DIR:-$HOME/.codex}"
    _E="${CODEX_PLUGIN_ROOT:-${PLUGIN_ROOT:-}}"
    _P=$({ [ -n "$_E" ] && printf '%s\n' "$_E"; ls -dt "$_C/plugins/cache/thedotmack/codex-mem"/[0-9]*/ 2>/dev/null; printf '%s\n' "$_C/plugins/marketplaces/thedotmack/plugin"; } | while …; done)
...
The codex-mem installer owns the file. Bake absolute paths via the helpers in `src/services/integrations/install-paths.ts` (Phase 3). On `codex-mem` upgrade, the installer must re-bake paths idempotently — see the migration logic in Phase 6.

### Rule C — Runtime resolution (`bun-runner.js`, `version-check.js`)

Both runtime scripts MUST accept `CODEX_PLUGIN_ROOT` env first, then fall back to `dirname(import.meta.url)/..`. This is the safety net behind Rules A and B.
```

**Verification checklist:**
- [ ] `CODEX.md` has a `## Spawn-Contract Resolution` section exactly as above.
...
export interface ShellTemplateOptions {
  ...
  // Example: ['node', '"$_P/scripts/bun-runner.js"', '"$_P/scripts/worker-service.cjs"', 'hook', 'codex-code', 'session-init']
  trailingCommand: string[];
  // Which host this is for. Selects the PATH-resolution prelude.
  host: 'codex-code' | 'codex-cli' | 'mcp';
  // Extra env exports prepended to the prelude (e.g. CODEX_MEM_CODEX_HOOK=1 for codex version-check).
...
1. **PATH-resolution prelude** (host-specific):
   - `codex-code`: `export PATH="$($SHELL -lc 'echo $PATH' 2>/dev/null):$PATH";` (matches `plugin/hooks/hooks.json:24`).
   - The Setup-hook variant has a hard-coded nvm path (`plugin/hooks/hooks.json:11`) — keep it as a special case `host: 'codex-code-setup'` or pass an `overridePathPrelude` field; reuse the literal from line 11.
...
3. **Candidate enumeration + filter loop** (reuse the existing pipeline from `plugin/hooks/hooks.json:24`):
   ...
   _P=$({
     [ -n "$_E" ] && printf '%s\n' "$_E";
     # MCP only: also try $PWD/plugin and $PWD and $HOME/.codex/plugins/cache/codex-mem-local/codex-mem/[0-9]*/
     ls -dt "$_C/plugins/cache/thedotmack/codex-mem"/[0-9]*/ 2>/dev/null;
...
5. **Cygpath conversion** (host-specific — `codex-code` and `codex-cli` only, NOT `mcp` because `sh -c` already runs under POSIX shell which understands POSIX paths):
   ```sh
...
6. **Extra env exports** (e.g. `CODEX_MEM_CODEX_HOOK=1` for codex version-check, see `plugin/hooks/codex-hooks.json:10`).

7. **Trailing command** (already shell-quoted by caller: `node "$_P/scripts/bun-runner.js" "$_P/scripts/worker-service.cjs" hook codex-code session-init`).
...
|---|---|---|
| `buildShellCommand({ host: 'codex-code-setup', requireFile: 'version-check.js', trailingCommand: ['node', '"$_P/scripts/version-check.js"'], notFoundMessage: 'codex-mem: version-check.js not found' })` | `plugin/hooks/hooks.json:11` | line 11 |
| `buildShellCommand({ host: 'codex-code', requireFile: 'bun-runner.js', requireFileSecondary: 'worker-service.cjs', trailingCommand: ['node', '"$_P/scripts/bun-runner.js"', '"$_P/scripts/worker-service.cjs"', 'start'], trailingJson: { continue: true, suppressOutput: true }, notFoundMessage: 'codex-mem: plugin scripts not found' })` | `plugin/hooks/hooks.json:24` | line 24 |
| (analogous for hooks.json:30, 42, 55, 68, 80) | each line in hooks.json | per line |
| `buildShellCommand({ host: 'codex-cli', requireFile: 'version-check.js', extraEnv: { CODEX_MEM_CODEX_HOOK: '1' }, trailingCommand: ['node', '"$_P/scripts/version-check.js"'], notFoundMessage: 'codex-mem: version-check.js not found' })` | `plugin/hooks/codex-hooks.json:10` | line 10 |
| (analogous for codex-hooks.json:15, 20, 32, 44, 56, 67) | each line | per line |
| `buildShellCommand({ host: 'mcp', requireFile: 'mcp-server.cjs', trailingCommand: ['exec', 'node', '"$_P/scripts/mcp-server.cjs"'], notFoundMessage: 'codex-mem: mcp server not found', mcpExtraCandidates: ['$PWD/plugin', '$PWD', '$HOME/.codex/plugins/cache/codex-mem-local/codex-mem/[0-9]*/'] })` | `.mcp.json:8` and `plugin/.mcp.json:8` | line 8 |
...
`shell-quote` (`scripts/build-hooks.js:101`, already a plugin runtime dep) provides `quote(words)` to safely escape `node`, `"$_P/scripts/X.cjs"`, `hook`, `codex-code`, `session-init`. Do not hand-build the string — escape via `quote()`.
...
- `getPluginRootAbsolutePath` — new logic: probe `process.env.CODEX_PLUGIN_ROOT`, then `MARKETPLACE_ROOT/plugin`, then `process.cwd()/plugin`, then `process.cwd()`. Document that this is install-time only (Rule B uses absolute paths; Rule C handles runtime).
...
- [ ] No installer file contains a string literal beginning with `${CODEX_PLUGIN_ROOT}` after this phase. Add a test:
  ```ts
  it('installers must not emit raw ${CODEX_PLUGIN_ROOT} placeholders', () => {
...
- ❌ Do not introduce new env vars (e.g. `CODEX_MEM_BUN_PATH`). The existing `findBunPath()` at `CursorHooksInstaller.ts:112–130` already handles platform variation; preserve that logic.
...
| 5 | `plugin/scripts/version-check.js` | C | No change — already correctly implemented at lines 7–17. Document in `CODEX.md`. |
| 6 | `plugin/scripts/bun-runner.js` | C | Document `RESOLVED_PLUGIN_ROOT` at line 11 in `CODEX.md`. **Keep `fixBrokenScriptPath` (lines 13–21)** — it's the runtime safety net for Rule A failures (the `_P` resolution lands on a wrong cache and the trailing `node "$_P/scripts/X.cjs"` arg becomes literal `/scripts/X.cjs`). Add a comment block explaining why it exists. |
...
```json
{ "command": "node ${CODEX_PLUGIN_ROOT}/scripts/your-hook.js" }
```

This is the canonical Codex Code documented form per upstream. **Keep the docs aligned with upstream** — do NOT replace these examples with the defensive shell prelude (which is codex-mem-internal complexity, not user-facing API).

Add a single subsection to `docs/public/hooks-architecture.mdx` titled "Why codex-mem's own hooks look different" that:
1. States the upstream contract: `${CODEX_PLUGIN_ROOT}` is set by the host.
2. Explains that codex-mem ships a defensive fallback because some host versions / cache rotations don't inject it.
...
- [ ] `git grep -n '\${CODEX_PLUGIN_ROOT}' -- ':(exclude)docs' ':(exclude)plugin/hooks' ':(exclude)*.mcp.json' ':(exclude)plans'` returns no hits — the only places that should mention raw `${CODEX_PLUGIN_ROOT}` are the host-managed shell-template files (Rule A) and user-facing docs.
- [ ] `npm test` passes.
...
- ❌ Do not "improve" docs by replacing `${CODEX_PLUGIN_ROOT}` with shell preludes — users would copy-paste shell complexity into single-purpose hooks that don't need it.
...
2. **No raw `${CODEX_PLUGIN_ROOT}` placeholder in installer-emitted JSON.** Scan the build output of `dist/npx-cli/index.js` for the literal substring `${CODEX_PLUGIN_ROOT}` (after esbuild bundling). It must not appear.
...
it('no installer-output JSON contains raw ${CODEX_PLUGIN_ROOT}', () => {
  ...
  // ~/.warp/mcp.json — none should contain the literal string '${CODEX_PLUGIN_ROOT}'.
});
```
...
Add a `lint:docs` script that fails CI if `docs/public/**/*.mdx` mentions `${CODEX_PLUGIN_ROOT}` in a `bash`/`sh` fenced code block (vs. JSON, which is the upstream-approved form).

```bash
# Pseudo-rule: any ```bash or ```sh block containing ${CODEX_PLUGIN_ROOT} fails.
# JSON examples are allowed because that's the upstream Codex Code hook contract.
```
...
**What to implement:** Handle existing installs in the wild that have absolute paths baked in from previous codex-mem versions. Plan the upgrade semantics for each integration.

### 6.1 Per-IDE migration matrix

| Integration | Current bake state | Migration on `npx codex-mem install` |
|---|---|---|
| Codex Code (Rule A) | host-managed; Codex Code rotates cache on `codex plugin update`. | No installer action needed. Setup hook (version-check.js) prints upgrade hint. Already implemented via `plans/2026-04-29-installer-streamline.md`. |
...
| Cursor (Rule B) | absolute paths in `~/.cursor/hooks.json` and `~/.cursor/mcp.json`. | `installCursorHooks` is idempotent (writes `hooks.json` whole); re-running `npx codex-mem install` re-bakes paths. |
| Gemini (Rule B) | absolute paths in `~/.gemini/settings.json`. | `mergeHooksIntoSettings` already overwrites the `codex-mem`-named hook entries (see `GeminiCliHooksInstaller.ts:97–123`) — re-running re-bakes. |
| Windsurf (Rule B) | absolute paths in `~/.codeium/windsurf/hooks.json`. | Idempotent rewrite — same pattern. |
| Copilot/Antigravity/Goose/Roo/Warp (Rule B) | absolute paths in each `mcp.json`. | `installMcpIntegration` overwrites `codex-mem` entry only (see `McpIntegrations.ts:31–39`). |
| OpenCode | absolute path of bundle copy. | `installOpenCodePlugin` overwrites the bundle file — `npm run build` then `npx codex-mem install` is the canonical upgrade path. |
| OpenClaw | configSchema-managed; no path baking. | No migration. |

### 6.2 Detection of stale installs

Add a new check in `npx codex-mem install` (in `src/npx-cli/commands/install.ts` setupIDEs flow): for each Rule B integration that's already installed, detect if the baked `mcpServerPath` / `workerServicePath` / `bunPath` still resolves on disk. If not, re-bake silently. Emit a single line: `Cursor: re-baked stale paths from <oldVersion> to <newVersion>`.

This addresses the case where a user installs codex-mem v12.7.0, then v12.8.0, and the v12.7.0 cache is still referenced in `~/.cursor/hooks.json` while the actual v12.7.0 bundle has been pruned by Codex Code's plugin garbage collector.
...
Cross-reference `plans/2026-05-06-codex-plugin-version-mismatch.md`: self-hosted Codex marketplaces need to re-add the marketplace post-codex-mem-upgrade because Codex 0.128 doesn't auto-upgrade enabled plugin caches. Add this note to:
- `docs/public/configuration.mdx` (Codex section if any)
- The "Spawn-Contract Resolution" section in `CODEX.md` (Phase 1) under a "Known limitations" subsection

**Verification checklist:**
- [ ] Re-running `npx codex-mem install` on a system with v(N-1) baked paths refreshes them to v(N) without user intervention.
...
**Anti-pattern guards:**
- ❌ Do not silently delete pre-existing user customizations in `~/.cursor/hooks.json` or `~/.gemini/settings.json`. Only overwrite the `codex-mem`-namespaced entries; preserve everything else (the existing installers already do this — verify it).
- ❌ Do not introduce a separate "migrate" CLI command. Keep migration implicit in `npx codex-mem install`.
...
- **12 IDEs:** codex-code, gemini-cli, opencode, openclaw, windsurf, codex-cli, cursor, copilot-cli, antigravity, goose, roo-code, warp.
- **N hook events per IDE** (per `src/cli/handlers/`):
  - codex-code: 6 (Setup, SessionStart, UserPromptSubmit, PreToolUse, PostToolUse, Stop).
...
- **3 resolution sources** (Rule A only): (a) host injects `CODEX_PLUGIN_ROOT`; (b) host doesn't inject, cache fallback hits; (c) host doesn't inject, cache fallback misses (must fail with the canonical "codex-mem: ... not found" stderr).
...
describe('Spawn-contract resolution — Rule A shell evaluation', () => {
  ...
  for (const file of ['plugin/hooks/hooks.json', 'plugin/hooks/codex-hooks.json']) {
    for (const command of commandHooksFrom(file)) {
      it(`[${file}] resolves _P when CODEX_PLUGIN_ROOT is set`, () => {
        const env = { CODEX_PLUGIN_ROOT: tmpPluginRoot, /* etc */ };
        const result = spawnSync('bash', ['-c', command + '; echo "_P=$_P"'], { env });
        expect(result.stdout.toString()).toContain(`_P=${tmpPluginRoot}`);
      });

      it(`[${file}] resolves _P from cache when CODEX_PLUGIN_ROOT is unset`, () => {
        // Set up tmp $HOME/.codex/plugins/cache/thedotmack/codex-mem/12.0.0/plugin/scripts/<requireFile>
        // Run command without CODEX_PLUGIN_ROOT; assert _P resolves to the cache path.
      });

      it(`[${file}] fails cleanly when no candidate exists`, () => {
        // Empty $HOME, no CODEX_PLUGIN_ROOT.
        const result = spawnSync('bash', ['-c', command], { env: { HOME: emptyTmpDir } });
        expect(result.status).not.toBe(0);
        expect(result.stderr.toString()).toMatch(/codex-mem: .* not found/);
      });
    }
  }
});
...
3. Assert no string in the config contains `${CODEX_PLUGIN_ROOT}` literally.
...
For each of the 12 IDEs, run `npx codex-mem install`, then start a session and verify:
- Codex Code: SessionStart hook fires; check via `~/.codex-mem/logs/`.
- Codex CLI: SessionStart hook fires; check via `~/.codex/logs/`.
- Cursor: `codex-mem` MCP server appears in MCP panel; one tool call succeeds.
- Gemini: `codex-mem` SessionStart hook runs; check via `~/.gemini/`.
- Windsurf: `codex-mem` hook runs.
- OpenCode: `codex-mem.js` plugin loads.
...
**Anti-pattern guards:**
- ❌ Do not skip the "fails cleanly when no candidate exists" test. The "codex-mem: ... not found" error is what users see when their install is broken; it's a contract.
...
3. Manual verification on a fresh machine for at least Codex Code + Codex + Cursor + 1 MCP-only IDE (per Phase 7.4).
...
1. Bump codex-mem version (the version-bump skill handles this).
2. Run `codex-mem version-bump` flow; the marketplace publishes the new bundle.
3. Watch for issues in the first 48 hours: monitor for any "codex-mem: <X> not found" reports in user issues — those signal Rule A fallback failures, which the test matrix should have caught.

### 8.3 Documentation deliverables (final)

After merge, confirm:

- `CODEX.md` has the `## Spawn-Contract Resolution` section (Phase 1.3).
- `docs/public/hooks-architecture.mdx` has the "Why codex-mem's own hooks look different" subsection (Phase 4.2).
...
**Anti-pattern guards:**
- ❌ Do not bypass version-bump (per CODEX.md "No need to edit the changelog ever, it's generated automatically.").
...
1. **Codex Code:** Is `CODEX_PLUGIN_ROOT` *guaranteed* to be set for hooks in plugin-loaded `hooks.json` files (vs. user-level `hooks.json`)? Source: Codex Code docs.
