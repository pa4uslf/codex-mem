...
> **Goal:** Stop host-side environment variables from contaminating the worker's Codex SDK subprocess. Two confirmed bugs anchor this plan: `CODEX_BASE_URL` leaks from the parent shell while `CODEX_AUTH_TOKEN` is blocked, breaking proxy/gateway auth (#2375); and `CODEX_CODE_EFFORT_LEVEL` propagates from host CLI settings into the SDK subprocess where it triggers a permanent HTTP 400 that the retry classifier mistakes for transient (#2357). Adjacent feature #2289 (`$TIER` alias syntax) is in scope where it shares the same env/model-resolution surface.
>
> **Net effect:**
> - The OAuth-skip predicate requires a real credential (`CODEX_API_KEY` or `CODEX_AUTH_TOKEN`), not a bare `CODEX_BASE_URL`. Proxy/gateway users put credentials in `~/.codex-mem/.env`; nothing relies on parent-shell leaks.
> - `BLOCKED_ENV_VARS` adds `CODEX_BASE_URL` and the `CODEX_CODE_EFFORT_LEVEL` / `CODEX_CODE_ALWAYS_ENABLE_EFFORT` pair (defense in depth alongside the existing `env-sanitizer.ts` `CODEX_CODE_*` prefix filter).
> - The Codex provider's error classifier explicitly handles HTTP 400 as `unrecoverable`, matching `GeminiProvider`/`OpenRouterProvider`. No more unbounded retry loop on permanent-error responses.
...
> - `~/.codex-mem/.env` becomes the single source of truth for non-OAuth Codex credentials. The loader's whitelist documents this contract.
>
> **Out of scope:**
...
### Bug A — `CODEX_BASE_URL` leaks, OAuth gets skipped, `CODEX_AUTH_TOKEN` is missing (#2375)
...
const BLOCKED_ENV_VARS = [
  'CODEX_API_KEY',       // #733
  'CODEX_AUTH_TOKEN',    // added 5edf1557 (2026-05-04) — leak prevention
  'CODEXCODE',
  'CODEX_CODE_OAUTH_TOKEN', // #2215
];
```

`CODEX_BASE_URL` is **not** in the list, so it survives `buildIsolatedEnv()` (lines 166–205) and reaches `isolatedEnv` from `process.env`.
...
if (
  ...
  isolatedEnv.CODEX_BASE_URL ||
  isolatedEnv.CODEX_AUTH_TOKEN
) {
  clearStaleMarker();
  return isolatedEnv;
}
...
- `CODEX_BASE_URL` ✅ (leaked from parent)
- `CODEX_AUTH_TOKEN` ❌ (blocked, never re-injected because `~/.codex-mem/.env` is empty for first-time proxy users)
- `CODEX_CODE_OAUTH_TOKEN` ❌ (skip path bypassed the keychain read)

Result: `Not logged in · Please run /login` from every SDK subprocess.

### Bug B — `CODEX_CODE_EFFORT_LEVEL` triggers permanent 400 + unbounded retry (#2357)

The Codex SDK subprocess reads `CODEX_CODE_EFFORT_LEVEL` from its env and forwards it as the `effort` parameter on Messages API calls. codex-mem's source contains **zero** references to `effort` — the leak path is environmental, not code. Models without effort support (Haiku 4.5, Sonnet 4.5, older) reject with HTTP 400.

`src/supervisor/env-sanitizer.ts` lines 1–51 already filters `CODEX_CODE_*` via `ENV_PREFIXES` (with explicit allowances in `ENV_PRESERVE`). But:

1. `buildIsolatedEnv` does NOT call `sanitizeEnv` internally; callers are expected to chain them.
2. `BLOCKED_ENV_VARS` is the canonical leak deny-list and does not name `CODEX_CODE_EFFORT_LEVEL`. Defense-in-depth is currently single-layer.
3. The retry classifier in `src/services/worker/CodexProvider.ts` has no HTTP 400 case; the default branch at line 98 returns `kind: 'transient'`, so a permanent 400 loops forever.

`src/services/worker/GeminiProvider.ts` lines 89–94 and `src/services/worker/OpenRouterProvider.ts` lines 82–87 already classify 400 as `unrecoverable`; that pattern is the copy-target for CodexProvider.

### Adjacent — `$TIER` alias syntax (#2289)

`src/shared/SettingsDefaultsManager.ts` line 116 already implements a *portable* `'haiku'` alias for `CODEX_MEM_TIER_SIMPLE_MODEL` (per #1463). What's missing is the user-facing `$TIER` *syntax* in the `CODEX_MEM_MODEL` field that resolves to a provider-appropriate model at request time. Same code surface (model resolution in `CodexProvider.getModelId` at lines 442–446); minimal extension.
...
| `loadCodexMemEnv` whitelist + `CodexMemEnv` interface | `src/shared/EnvManager.ts:26–32, 79–100` | Single source of truth for what `~/.codex-mem/.env` accepts |
| `ENV_PRESERVE` / `ENV_EXACT_MATCHES` / `ENV_PREFIXES` | `src/supervisor/env-sanitizer.ts:1–51` | Whitelist-based env stripping; do NOT add `CODEX_CODE_EFFORT_LEVEL` to `ENV_PRESERVE` |
| Provider error classifier (HTTP 400 → unrecoverable) | `src/services/worker/GeminiProvider.ts:89–94`, `src/services/worker/OpenRouterProvider.ts:82–87` | Identical pattern to apply in `CodexProvider` |
...
- codex-mem source has **zero references** to `effort`, `CODEX_CODE_EFFORT_LEVEL`, `CODEX_CODE_ALWAYS_ENABLE_EFFORT`, or `reasoning_effort`. Do not "remove the effort parameter we forward" — there is none. The leak is the SDK subprocess reading the env var directly.
...
- `sanitizeEnv` is whitelist-based (preserves a fixed set; strips everything matching `CODEX_CODE_*` etc). It is NOT idempotent if you re-add a name to `ENV_PRESERVE`. Do not add `CODEX_CODE_EFFORT_LEVEL` to `ENV_PRESERVE` — that's the opposite of what we want.
...
- The `~/.codex-mem/.env` loader at `src/shared/EnvManager.ts:79–100` uses property-by-property assignment as an implicit whitelist. Do NOT replace with `Object.assign(result, parsed)` — that breaks the whitelist guarantee.
...
| `src/supervisor/env-sanitizer.ts` | 51 | Light edit (Phase 3 — comment change only; `CODEX_CODE_*` prefix already filters EFFORT_LEVEL) |
| `src/services/worker/CodexProvider.ts` | 448 | Edited (Phase 3 — error classifier on `query()` rejection path) |
...
| `CODEX.md` | small | Edited (Phase 7 — document `~/.codex-mem/.env` contract) |
...
Use `bun:test` per `package.json` `"test": "bun test"`. Pattern from `tests/codex-provider-resume.test.ts:1`.

1. **`buildIsolatedEnvWithFreshOAuth strips CODEX_BASE_URL when no .env credentials are configured`**
   - Stub `process.env.CODEX_BASE_URL = 'https://proxy.example'`, no `~/.codex-mem/.env`, no API_KEY/AUTH_TOKEN in env.
   - Call `buildIsolatedEnvWithFreshOAuth()`.
   - Assert: result does NOT have `CODEX_BASE_URL` (post-fix). Currently RED.
2. **`OAuth-skip does not fire on bare CODEX_BASE_URL`**
   - Same setup. Spy on `readCodexOAuthToken`.
   - Assert: `readCodexOAuthToken` was called (because BASE_URL alone is not enough to skip). Currently RED — `readCodexOAuthToken` is NOT called today.
3. **`CODEX_AUTH_TOKEN from ~/.codex-mem/.env reaches the isolated env`**
   - Write a temp `.env` with `CODEX_AUTH_TOKEN=test-token` and `CODEX_BASE_URL=https://proxy.example`.
   - Assert: `isolatedEnv.CODEX_AUTH_TOKEN === 'test-token'` AND `isolatedEnv.CODEX_BASE_URL === 'https://proxy.example'`. Currently GREEN (already works); test guards against regression.
4. **`CODEX_CODE_EFFORT_LEVEL is stripped from the isolated env`**
   - Stub `process.env.CODEX_CODE_EFFORT_LEVEL = 'MAX'`.
   - Assert: `sanitizeEnv(buildIsolatedEnv())` does NOT contain `CODEX_CODE_EFFORT_LEVEL`. Currently GREEN via `env-sanitizer.ENV_PREFIXES`; test guards.
5. **`CODEX_CODE_EFFORT_LEVEL is in BLOCKED_ENV_VARS for defense-in-depth`**
   - Assert: `BLOCKED_ENV_VARS.includes('CODEX_CODE_EFFORT_LEVEL')`. Currently RED.
6. **`HTTP 400 from Codex SDK is classified unrecoverable`**
   - Construct an error matching the SDK's 400 shape (`error.status === 400`, body contains `does not support the effort parameter`).
   - Assert: `classifyCodexProviderError(err).kind === 'unrecoverable'`. Currently RED — falls through to `transient`.
7. **`HTTP 400 with effort-parameter body emits a once-only warn log`**
   - Same setup as 6, plus capture `logger.warn` calls.
   - Assert: warn fires once with category `SDK` and a hint pointing at #2357 / `~/.codex-mem/.env`. Currently RED.
...
**Goal:** Make the OAuth-skip require a real credential, and add `CODEX_BASE_URL` to the deny-list so it can only be configured via `~/.codex-mem/.env`.
...
**After (add `CODEX_BASE_URL`):**
```ts
const BLOCKED_ENV_VARS = [
  'CODEX_API_KEY',       // #733
  'CODEX_AUTH_TOKEN',    // 5edf1557 — leak prevention; re-injected from ~/.codex-mem/.env when configured
  'CODEX_BASE_URL',      // #2375 — same leak class as AUTH_TOKEN; re-injected from ~/.codex-mem/.env. Without this entry, a leaked BASE_URL alone triggered the OAuth-skip while no auth credential reached the subprocess.
  'CODEXCODE',
  'CODEX_CODE_OAUTH_TOKEN', // #2215
];
```
...
// CODEX_BASE_URL is not a credential — every documented gateway needs
// either an AUTH_TOKEN or an API_KEY. This guards #2375 against a class of
// leaks where a parent shell exports BASE_URL (e.g. for the Codex Code CLI
...
### 2.3 Verify the `~/.codex-mem/.env` re-injection at `src/shared/EnvManager.ts:178–195`
...
```ts
// Contract (post-#2375): CODEX_BASE_URL, CODEX_AUTH_TOKEN, and
// CODEX_API_KEY are *only* populated from ~/.codex-mem/.env. They are
...
- [ ] `grep -n "CODEX_BASE_URL" src/shared/EnvManager.ts` shows entries at: `BLOCKED_ENV_VARS`, `CodexMemEnv` interface, loader, re-injection, OAuth-skip predicate (NOT in skip predicate).
- [ ] Smoke: with a `~/.codex-mem/.env` containing `CODEX_BASE_URL=...` and `CODEX_AUTH_TOKEN=...`, the worker actually authenticates against the proxy. Test with BigModel or any sandboxed proxy.

### 2.5 Anti-pattern guards

- Do NOT add `CODEX_BASE_URL` to `ENV_PRESERVE` in `env-sanitizer.ts` — `BLOCKED_ENV_VARS` is the right layer; `env-sanitizer` is a downstream filter.
...
- Do NOT delete the existing `delete isolatedEnv.CODEX_CODE_OAUTH_TOKEN` defensive line at line 229. That guard is intact; it's belt-and-suspenders for #2215 and orthogonal to this plan.

---

## Phase 3 — Fix #2357 (CODEX_CODE_EFFORT_LEVEL leak + 400 retry classification)

**Goal:** Two-layer defense for the env leak (existing `CODEX_CODE_*` prefix filter + new `BLOCKED_ENV_VARS` entries), plus a permanent classification for the resulting HTTP 400 so the retry loop terminates if the leak ever sneaks past either layer.
...
const BLOCKED_ENV_VARS = [
  ...
  // env-sanitizer's CODEX_CODE_* prefix filter strips these for spawn paths
  // that go through it, but BLOCKED_ENV_VARS is the canonical deny-list and
  // belongs in defense-in-depth.
  'CODEX_CODE_EFFORT_LEVEL',
  'CODEX_CODE_ALWAYS_ENABLE_EFFORT',
];
```

### 3.2 Edit `src/services/worker/CodexProvider.ts` — classify HTTP 400 as unrecoverable

Locate the existing error-classification path. The Codex SDK raises errors with `error.status` and a body containing the failure description. Pattern from `src/services/worker/GeminiProvider.ts:89–94` (the canonical copy-target):
...
Add the equivalent in `CodexProvider`'s error classifier (new function or existing — read the file; create if absent, mirroring `GeminiProvider` shape):

```ts
function classifyCodexProviderError(input: { cause: unknown }): ClassifiedProviderError {
  ...
  // is a leaked CODEX_CODE_EFFORT_LEVEL the SDK subprocess forwarded as
  ...
  if (status === 400) {
    if (/effort parameter/i.test(bodyText)) {
      logger.warn(
        'SDK',
        'Codex API rejected effort parameter — likely CODEX_CODE_EFFORT_LEVEL leaked into SDK env (issue #2357). Configure CODEX_MEM_MODEL or set credentials in ~/.codex-mem/.env.',
        { status, bodyText },
      );
    }
    return new ClassifiedProviderError(
      `Codex bad request (status 400): ${bodyText}`,
      { kind: 'unrecoverable', cause: input.cause },
    );
  }
  ...
  if (status === 401 || status === 403) {
    return new ClassifiedProviderError(
      `Codex auth rejected (status ${status})`,
  ...
  return new ClassifiedProviderError(
    `Codex SDK error: ${bodyText}`,
    { kind: 'transient', cause: input.cause },
  );
}
```

Wire this classifier into the existing `try { ... } catch` around `query(...)` in `CodexProvider.ts`. **Read the actual catch shape before editing** — the function lives near line 180–195 and the existing `for await` over `queryResult` is where rejections surface.

### 3.3 Confirm `src/supervisor/env-sanitizer.ts` already strips `CODEX_CODE_EFFORT_LEVEL`

Read lines 1–51. Verify:
- `ENV_PREFIXES` includes `'CODEX_CODE_'`.
- `ENV_PRESERVE` does NOT include `CODEX_CODE_EFFORT_LEVEL`, `CODEX_CODE_ALWAYS_ENABLE_EFFORT`.

Add an inline comment at the `ENV_PREFIXES` declaration:

```ts
// Filters CODEX_CODE_* unless explicitly preserved in ENV_PRESERVE.
...
- [ ] `grep -n "CODEX_CODE_EFFORT_LEVEL" src/` returns hits in `EnvManager.ts` (BLOCKED_ENV_VARS) and the test file. Nothing else.
- [ ] Reproduce #2357 scenario locally:
  ```bash
  CODEX_CODE_EFFORT_LEVEL=MAX bun run src/services/worker-service.ts --daemon
...
- Do NOT log inside `classifyCodexProviderError` for every 400 — only the effort-parameter sub-case warrants a hint. Generic 400s are noisy enough at the call site.
...
- Do NOT rely on the SDK supporting an `effort` SDK-option that we strip. The SDK type does not expose `effort`; the leak is the SDK's own subprocess (`pathToCodexCodeExecutable`) reading the env var. Stripping at our env layer is the only fix we control.

---

## Phase 4 — `$TIER` alias syntax (#2289)

**Goal:** Allow `CODEX_MEM_MODEL=$TIER:summary` (and similar) to resolve at request time to a provider-appropriate model, reusing the existing `'haiku'` portable alias machinery (line 116, #1463). Optional phase; can be deferred without blocking Phase 2/3.
...
```ts
CODEX_MEM_TIER_FAST_MODEL: string;     // for $TIER:fast — defaults to 'haiku'
CODEX_MEM_TIER_SMART_MODEL: string;    // for $TIER:smart — defaults to 'sonnet' (or provider-equivalent)
```

Add to the `DEFAULTS` block near lines 115–117:

```ts
CODEX_MEM_TIER_FAST_MODEL: 'haiku',
CODEX_MEM_TIER_SMART_MODEL: 'sonnet',
```

### 4.2 Edit `src/services/worker/CodexProvider.ts:442–446` — add `$TIER` resolution
...
private getModelId(): string {
  ...
  return resolveTierAlias(settings.CODEX_MEM_MODEL, settings);
}
```
...
export function resolveTierAlias(model: string, settings: SettingsDefaults): string {
  ...
  switch (match[1]) {
    case 'fast':    return settings.CODEX_MEM_TIER_FAST_MODEL || 'haiku';
    case 'smart':   return settings.CODEX_MEM_TIER_SMART_MODEL || 'sonnet';
    case 'simple':  return settings.CODEX_MEM_TIER_SIMPLE_MODEL || 'haiku';
    case 'summary': return settings.CODEX_MEM_TIER_SUMMARY_MODEL || settings.CODEX_MEM_MODEL;
...
- [ ] New test: `resolveTierAlias('$TIER:fast', settings)` returns `settings.CODEX_MEM_TIER_FAST_MODEL`.
- [ ] New test: `resolveTierAlias('codex-haiku-4-5-20251001', settings)` returns input unchanged (non-tier passthrough).
- [ ] Setting `CODEX_MEM_MODEL=$TIER:fast` and starting the worker actually queries against the fast-tier model.
...
**Goal:** Every place codex-mem spawns a subprocess must apply both `buildIsolatedEnv` (or the async variant) AND `sanitizeEnv`. A grep-based check codifies the rule.

### 5.1 Audit table — current state per call site

| File | Line | Spawn target | Env construction | Sufficient? |
|---|---|---|---|---|
| `src/services/worker/CodexProvider.ts` | 155 | Codex SDK subprocess | `sanitizeEnv(await buildIsolatedEnvWithFreshOAuth())` | ✅ |
...
| `src/services/infrastructure/ProcessManager.ts` | 415 | Worker daemon | `sanitizeEnv({...process.env, CODEX_MEM_WORKER_PORT, ...extraEnv})` | ⚠️ daemon inherits parent env then sanitizes — does not pass through `buildIsolatedEnv`. **Document why this is OK**: daemon is the trust boundary; parent env IS the truth. But it should still strip `CODEX_CODE_EFFORT_LEVEL` via the prefix filter. Confirm. |
...
For the worker-daemon and downstream MCP/chroma spawns, parent-process env IS the source of truth — they are pre-credential paths. As long as `CODEX_CODE_EFFORT_LEVEL` and the Codex credentials are stripped (which `sanitizeEnv` does via `CODEX_CODE_*` prefix and the existing `CODEX_AUTH_TOKEN` block), behavior is correct. The plan does not change these paths — it adds tests that prove they stay correct.
...
9. **`worker-daemon spawn env does not contain CODEX_CODE_EFFORT_LEVEL`**
   - Stub `process.env.CODEX_CODE_EFFORT_LEVEL = 'MAX'`.
   - Construct the env block as ProcessManager.ts:415 does.
   - Assert: result does not contain `CODEX_CODE_EFFORT_LEVEL`. Currently GREEN.
...
```bash
# Setup:
cat > ~/.codex-mem/.env <<'EOF'
CODEX_BASE_URL=https://open.bigmodel.cn/api/codex
CODEX_AUTH_TOKEN=<your-bigmodel-token>
EOF
chmod 600 ~/.codex-mem/.env
...
# Trigger:
# In any Codex Code session, use any tool — PostToolUse hook should land an observation.

# Verify:
tail -f ~/.codex-mem/logs/codex-mem-$(date +%Y-%m-%d).log
# Expect: no "Not logged in" errors; observations land via the proxy.
```

### 6.2 Manual smoke (CODEX_CODE_EFFORT_LEVEL leak)

```bash
# Setup:
export CODEX_CODE_EFFORT_LEVEL=MAX
export CODEX_CODE_ALWAYS_ENABLE_EFFORT=true

# Restart Codex Code so the env propagates to the hook subprocess.
...
- [ ] `bun test` is green.
- [ ] One iteration on a fresh machine confirms `~/.codex-mem/.env` is the only knob users need for proxy auth.
...
### 7.2 Edit `CODEX.md` — document the `~/.codex-mem/.env` contract

Add a section under "Configuration":

```markdown
### Codex Credentials (proxies, gateways, BigModel, etc.)

For non-OAuth Codex credentials (proxies / gateways / `CODEX_AUTH_TOKEN` / `CODEX_API_KEY`), put them in `~/.codex-mem/.env`:

\```
CODEX_BASE_URL=https://your-proxy.example
CODEX_AUTH_TOKEN=your-token
\```
...
- [ ] CODEX.md section exists and accurately reflects the new contract.
...
- Do NOT add the `.env` file path to `.gitignore` — it lives in `~/.codex-mem/`, not in the repo, so it's already outside.
...
- [ ] Read `node_modules/@codex-ai/codex-agent-sdk/sdk.d.ts` to confirm `query()` options does NOT support `effort` natively. If the SDK adds it, Phase 3.2's body-text regex still works as a fallback, but a code-level strip becomes the right fix.
- [ ] Verify `~/.codex-mem/.env` permissions are `0o600` post-fix (the saver enforces this; readers should not weaken it).
