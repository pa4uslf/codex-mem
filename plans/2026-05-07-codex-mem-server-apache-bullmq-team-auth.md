# Codex-Mem Server: Apache-2.0, BullMQ, Team Auth Plan

Status: implementation plan
Date: 2026-05-07
Primary command: `codex-mem server`
...
Build Codex-Mem Server inside this repo as the canonical next runtime. Keep the current worker as a compatibility shim while moving shared server logic into typed server/core/storage modules.
...
- Do not create a second repo or primary `codex-mem-server` package.
...
- Do not silently fall back to SQLite when `CODEX_MEM_QUEUE_ENGINE=bullmq` is explicitly configured.
...
- Use scope and commercial boundary from `/Users/alexnewman/Downloads/codex-mem-handoff-docs/apache-2-plan.md`.

Verification:

- `rg -n "AGPL|GNU Affero|Affero|GPL|copyleft|license" .`
- `rg -n "Codex-Mem™|trademark|official Codex|endorsed by Codex" .`
...
- Do not add trademark claims around `Codex-Mem`.
...
- Teach `src/npx-cli/index.ts` to route:
  - `codex-mem server start`
  - `codex-mem server stop`
  - `codex-mem server restart`
  - `codex-mem server status`
  - `codex-mem server doctor`
  - `codex-mem server logs`
  - `codex-mem server migrate`
  - `codex-mem server export`
  - `codex-mem server import`
  - `codex-mem server api-key create|list|revoke`
- Keep existing `start|stop|restart|status` as worker compatibility aliases.
- Add `codex-mem worker start|stop|restart|status` aliases that call the same command implementation as `server`.
...
- Do not remove `npx codex-mem install`.
...
- Use data contracts from `/Users/alexnewman/Downloads/codex-mem-handoff-docs/codex-mem-server-plan.md`.
...
- Preserve provider contract consumed by Codex/Gemini/OpenRouter providers through `SessionManager`.
...
- Add settings:
  ...
  - `CODEX_MEM_REDIS_URL`
  - `CODEX_MEM_REDIS_HOST`
  - `CODEX_MEM_REDIS_PORT`
  - `CODEX_MEM_REDIS_MODE=external|managed|docker`
...
- Integration tests gated by `CODEX_MEM_REDIS_URL`.
...
- Create `src/server/middleware/auth.ts`:
  ...
  - allow localhost unauthenticated reads only if `CODEX_MEM_AUTH_MODE=local-dev`.
- Create CLI commands:
  - `codex-mem server api-key create --team <team> --scope memories:read,memories:write`
  - `codex-mem server api-key list`
  - `codex-mem server api-key revoke <id>`
...
- Copy endpoint list from `codex-mem-server-plan.md`.
...
- Add `src/adapters/codex-code/mapper.ts` to map existing hook payloads to `AgentEvent`.
...
- Add mapper tests for Codex Code, Codex transcript watcher, and generic REST event payloads.

Anti-pattern guards:

- Do not make Codex Code the core data model.
...
- Update README to introduce Codex-Mem Server first and worker as compatibility language.
- Document:
  - local dev mode;
  - Docker deployment;
  - API-key creation;
...
- Do not call Codex-Mem an official Codex project.
...
- `npx codex-mem install` still works.
- `codex-mem server start|status|stop` works.
- `codex-mem worker start|status|stop` aliases work.
- Existing Codex Code hooks still write observations.
