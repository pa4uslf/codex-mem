# Codex-Mem 13 Server Beta: Worker Parity Plus Team Features

Status: implementation plan
Date: 2026-05-07
Release target: codex-mem 13
Primary goal: add `server (beta)` as an installer-selectable runtime while leaving the existing worker in place
Relationship to prior plan: follows `plans/2026-05-07-codex-mem-server-apache-bullmq-team-auth.md`, but narrows the release strategy to full worker parity plus additive team features

## Executive Decision

Codex-Mem 13 should ship **Server (beta)** as an opt-in runtime, not as a worker replacement.

The existing worker remains:

- the default installer runtime;
- the stable compatibility path for current users;
- the implementation that current Codex Code hooks can continue to call;
...
- `/Users/alexnewman/Downloads/codex-mem-handoff-docs/apache-2-plan.md`
...
- `docker/codex-mem/entrypoint.sh`
...
- Do not remove, rename, or deprecate the worker in codex-mem 13.
...
- Do not claim full team sync, hosted cloud, SSO, billing, or enterprise admin UI in codex-mem 13.
...
- `npx codex-mem update`
- `npx codex-mem uninstall`
- `npx codex-mem start`
- `npx codex-mem stop`
- `npx codex-mem restart`
- `npx codex-mem status`
- `npx codex-mem search <query>`
- `npx codex-mem adopt [--dry-run] [--branch <name>]`
- `npx codex-mem cleanup [--dry-run]`
- `npx codex-mem transcript watch`
...
- Codex/Gemini/OpenRouter providers
...
- Persist runtime settings:
  - `CODEX_MEM_RUNTIME=worker|server-beta`
  - `CODEX_MEM_SERVER_BETA_ENABLED=true|false`
  - `CODEX_MEM_SERVER_PORT`
  - `CODEX_MEM_SERVER_HOST`
  - `CODEX_MEM_QUEUE_ENGINE=sqlite|bullmq`
- For codex-mem 13, default to worker in interactive and non-interactive installs unless the user explicitly selects Server beta.
- If Server beta is selected, still install the worker bundle and keep worker commands available.

Documentation references:

- Copy prompt style from `promptProvider()` and `promptCodexModel()` in `src/npx-cli/commands/install.ts`.
...
- Verify `npx codex-mem install --no-auto-start` still skips worker/server startup.
...
- Add `codex-mem server start|stop|restart|status|doctor|logs`.
- Keep `codex-mem start|stop|restart|status` mapped to the selected runtime, but in codex-mem 13 default that selection is worker.
...
- `codex-mem worker status` works with existing worker.
- `codex-mem server status` does not lie when server beta is not installed.
...
- Shared queue contract test suite:
  ...
  - queue depth;
  - clear on response.
- BullMQ tests gated by `CODEX_MEM_REDIS_URL` or Docker Compose.
...
- Add Better Auth only to Server beta in codex-mem 13.
...
- Add API-key CLI:
  - `codex-mem server api-key create`
  - `codex-mem server api-key list`
  - `codex-mem server api-key revoke`
- Add team CLI:
  - `codex-mem server team create`
  - `codex-mem server team list`
  - `codex-mem server team invite`
  - `codex-mem server team members`
  - `codex-mem server team switch`
- Add permissions:
  - `memories:read`
  - `memories:write`
  - `memories:forget`
...
- Do not enable Better Auth on the worker stable path in codex-mem 13.
...
- Use data contracts from `codex-mem-server-plan.md`.
...
- Keep plugin hooks calling `worker-service.cjs` in codex-mem 13 by default.
- When installer selects Server beta, write setting `CODEX_MEM_RUNTIME=server-beta` and have hook handlers route to server beta only after server beta health is confirmed.
...
- Do not turn the viewer into an enterprise admin console in codex-mem 13.
...
- Add Docker Compose profile for Server beta:
  - `codex-mem-server-beta`
  - `valkey`
  - optional `chroma`
- Keep existing `docker/codex-mem` harness available.
- Persist:
  - SQLite data;
  - logs;
  - Valkey AOF data;
...
- Copy current Docker auth/credential mounting from `docker/codex-mem/entrypoint.sh` and `run.sh`.
...
- Add changelog language for codex-mem 13:
...
- Worker is still default after `npx codex-mem install`.
- Installer can select `Server (beta)`.
- `codex-mem worker start|status|stop` works.
- `codex-mem server start|status|stop` works.
- Current Codex Code hooks continue working in worker mode.
...
The key release rule: **codex-mem 13 can ship Server beta only when worker remains fully intact and Server beta has an explicit parity test report.**
