...
Codex-Mem operates as a Codex Code plugin with the following core components:
...
2. **Worker Service** - Express HTTP API on a per-user port; processes observations via the Codex CLI (or Gemini / OpenRouter)
...
| **AI SDK**             | Codex CLI (or Gemini / OpenRouter) |
...
1. **Input**: Codex Code sends tool execution data via stdin to hooks
...
### Search Pipeline
```
User Query → MCP Tools Invoked → HTTP API → SessionSearch Service → FTS5 Database → Search Results → Codex
```

1. **User Query**: User asks naturally: "What bugs did we fix?"
2. **MCP Tools Invoked**: Codex recognizes intent and invokes MCP search tools
...
6. **Return**: Codex presents formatted results to user
...
│    "run: npx codex-mem repair" to stderr. Always exits 0.      │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
...
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│ 3. Codex Uses Tools → PostToolUse Hook Fires (100+ times)      │
│    Captures tool executions, sends to worker for AI compression │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│ 4. Worker Processes → Codex CLI Analyzes                 │
│    Extracts structured learnings via iterative AI processing    │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│ 5. Codex Stops → Summary Hook Fires                            │
...
│   ├── .codex-legacy-plugin/plugin.json
...
The plugin registers a Setup-phase `version-check.js` plus five lifecycle hooks. Each lifecycle event invokes `bun-runner.js` to spawn `worker-service.cjs` with a `hook codex-code <event>` argument; the worker process is the single dispatcher for all hook logic. Events:

- **Setup** → `version-check.js` (sub-100ms marker check; never installs anything)
- **SessionStart** → start worker, then `hook codex-code context` (context injection)
...
- **Stop** → `hook codex-code summarize` (summary generation)

The actual runtime install (Bun, uv, `bun install`) is performed by `npx codex-mem install` / `npx codex-mem repair` with a visible installer spinner; the Setup hook itself only reads the `.install-version` marker.

See [Plugin Hooks](/architecture/hooks) for detailed hook documentation.

### 2. Worker Service
Express.js HTTP server on a per-user port (default `37700 + (uid % 100)`, override via `CODEX_MEM_WORKER_PORT`) with:
- Search HTTP API endpoints
- Viewer UI HTTP/SSE endpoints
- Async observation processing via the Codex CLI (or Gemini / OpenRouter)
