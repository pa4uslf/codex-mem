...
Codex has access to 4 MCP tools. When searching memory, Codex follows the 3-layer workflow:
...
Worker returns structured data → MCP server → Codex:
...
### 6. Codex Processes Results

Codex reviews the index, decides which observations are relevant, and can:
...
Always visible to Codex. Explains the 3-layer workflow pattern.
...
**Purpose:** Ensures Codex follows token-efficient pattern
...
**Location:** `/Users/YOUR_USERNAME/.codex/plugins/marketplaces/thedotmack/plugin/scripts/mcp-server.cjs`
...
**Before:** Progressive disclosure was something Codex had to remember
...
**Both clients use the same MCP tools** - the architecture works identically for Codex Desktop and Codex Code.
...
- ~10x token savings
- 100% relevance (Codex chooses what to fetch)
