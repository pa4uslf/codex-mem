...
The worker service is a long-running HTTP API built with Express.js and managed natively by Bun. It processes observations through the Codex CLI separately from hook execution to prevent timeout issues.
...
- **Port**: Per-user default `37700 + (uid % 100)` (override with `CODEX_MEM_WORKER_PORT`). The active port is stored in `~/.codex-mem/settings.json` and reported by `GET /api/health`.
- **Location**: `src/services/worker-service.ts`
- **Built Output**: `plugin/scripts/worker-service.cjs`
- **Model**: Configurable via `CODEX_MEM_MODEL` (default: `gpt-5-mini`)
...
The `port` value is the actual worker port for the current user — per-user default `37700 + (uid % 100)`, or whatever `CODEX_MEM_WORKER_PORT` is set to. The example above is illustrative; your value will differ.
...
Bun is required to run the worker service. If Bun is not installed, `npx codex-mem install` (and `npx codex-mem repair`) installs it globally during setup, with a visible clack spinner:
...
The worker service routes observations to the Codex CLI for AI-powered processing:

### Processing Flow

1. **Observation Queue**: Observations accumulate in memory
2. **SDK Processing**: Observations sent to Codex via Codex CLI
...
- **Parser** (`src/sdk/parser.ts`): Parses Codex's XML responses
- **Worker** (`src/sdk/worker.ts`): Main SDK agent loop

### Model Configuration

Set the Codex model used for compression via environment variable or `~/.codex-mem/settings.json`:

```bash
export CODEX_MEM_MODEL=gpt-5-mini
```

Allowed values:
- `gpt-5-mini` - default, fast and cheap (best for compression)
...
- **Override**: Set `CODEX_MEM_WORKER_PORT` (env or `~/.codex-mem/settings.json`)
- **Discovery**: `GET /api/health` returns the active port; `~/.codex-mem/settings.json` stores the configured value

If the chosen port is occupied, the worker fails to start — pin a different port via `CODEX_MEM_WORKER_PORT` and restart.
