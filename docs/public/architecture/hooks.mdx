...
Codex-Mem implements a **5-stage hook system** that captures development work across Codex Code sessions. This document provides a complete technical reference for developers implementing this pattern on other platforms.

## Architecture Overview

### System Architecture

This two-process architecture works in both Codex Code and VS Code:
...
graph TB
    ...
    subgraph WORKER["Worker Process (separate Node.js)"]
        ...
        AGENT -->|Event-Driven| CODEX[Codex CLI]
        CODEX --> SQLITE[(SQLite + FTS5)]
        CODEX --> CHROMA[(Chroma Vectors)]
    end
...
graph TB
    ...
    H --> I["Call Codex API<br/>(compress observation)"]
...
|-------|------|---------|---------|
| **1. SessionStart** | `context-hook.js` | User opens Codex Code | Inject prior context silently |
| **2. UserPromptSubmit** | `new-hook.js` | User submits a prompt | Create/get session, save prompt, init worker |
| **3. PostToolUse** | `save-hook.js` | Codex uses any tool | Queue observation for AI compression |
...
```json
{
  "hooks": {
    ...
    "SessionStart": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "bun ${CODEX_PLUGIN_ROOT}/scripts/worker-service.cjs start",
        "timeout": 60
      }, {
        "type": "command",
        "command": "bun ${CODEX_PLUGIN_ROOT}/scripts/context-hook.js",
        "timeout": 60
      }]
    }],
    "UserPromptSubmit": [{
      "hooks": [{
        "type": "command",
        "command": "node ${CODEX_PLUGIN_ROOT}/scripts/new-hook.js",
        "timeout": 120
      }]
    }],
    "PostToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "node ${CODEX_PLUGIN_ROOT}/scripts/save-hook.js",
        "timeout": 120
      }]
    }],
    "Stop": [{
      "hooks": [{
        "type": "command",
        "command": "node ${CODEX_PLUGIN_ROOT}/scripts/summary-hook.js",
        "timeout": 120
      }]
    }],
    "SessionEnd": [{
      "hooks": [{
        "type": "command",
        "command": "node ${CODEX_PLUGIN_ROOT}/scripts/cleanup-hook.js",
        "timeout": 120
      }]
    }]
  }
...
**Timing**: When user opens Codex Code or resumes session
...
(Runtime setup is handled out-of-band by `npx codex-mem install` / `npx codex-mem repair`. The Setup phase runs a sub-100ms `version-check.js` that prompts the user to repair if the `.install-version` marker is stale.)
...
sequenceDiagram
    ...
    alt Worker Ready
        ...
        IDE->>IDE: Inject context to Codex's prompt
...
**Purpose**: Inject context from previous sessions into Codex's initial context.
...
sequenceDiagram
    ...
    alt Prompt fully private (empty after stripping)
        ...
        NewHook->>DB: INSERT OR IGNORE INTO sdk_sessions<br/>(codex_session_id, project, first_user_prompt)
...
**Timing**: After Codex uses any tool (Read, Bash, Grep, Write, etc.)
...
sequenceDiagram
    ...
    Codex->>IDE: Uses tool: Read("/src/auth.ts")
    ...
    alt Tool in skip list
        ...
        SaveHook->>Worker: POST /api/sessions/observations<br/>{ codexSessionId, tool_name, tool_input, tool_response, cwd }<br/>(fire-and-forget, 2s timeout)

        SaveHook-->>IDE: { continue: true, suppressOutput: true }
        IDE-->>Codex: Tool execution complete

        Note over Worker,DB: Async path (doesn't block IDE)

        Worker->>Worker: createSDKSession(codexSessionId)<br/>→ returns sessionDbId
        Worker->>Worker: Check if prompt was private<br/>(skip if fully private)
        Worker->>Agent: Queue observation for processing
        Agent->>Agent: Call Codex CLI to compress<br/>observation into structured format
...
**Key Pattern:** The hook returns immediately after HTTP POST. AI compression happens asynchronously in the worker without blocking Codex's tool execution.
...
**Worker Processing**:
1. Looks up or creates session: `createSDKSession(codexSessionId, '', '')`
...
6. SDK agent calls Codex to compress into structured observation
...
sequenceDiagram
    ...
    SummaryHook->>Worker: POST /api/sessions/summarize<br/>{ codexSessionId, last_user_message, last_assistant_message }<br/>(fire-and-forget, 2s timeout)
    ...
    Worker->>Worker: Lookup sessionDbId from codexSessionId
    Worker->>Agent: Queue summarization request
    Agent->>Agent: Call Codex CLI with prompt:<br/>"Summarize: request, investigated, learned, completed, next_steps"
...
2. Agent calls Codex to generate structured summary
...
**Timing**: When Codex Code session closes (exit, clear, logout, etc.)
...
sequenceDiagram
    ...
    CleanupHook->>Worker: POST /api/sessions/complete<br/>{ codexSessionId, reason }<br/>(fire-and-forget, 2s timeout)
    ...
    Worker->>Worker: Lookup sessionDbId from codexSessionId
    Worker->>DB: UPDATE sdk_sessions<br/>SET status = 'completed', completed_at = NOW()<br/>WHERE codex_session_id = codexSessionId
...
**Worker Processing**:
1. Finds session by `codexSessionId`
...
stateDiagram-v2
    ...
    note right of Active
        session_id: constant (e.g., "codex-session-abc123")
...
erDiagram
    ...
    SDK_SESSIONS {
        ...
        text codex_session_id UK "From IDE (e.g., 'codex-session-123')"
...
INSERT OR IGNORE INTO sdk_sessions (codex_session_id, project, first_user_prompt)
VALUES (?, ?, ?)
RETURNING id;
...
### SDK Integration
- [ ] Call Codex CLI to process observations/summaries
