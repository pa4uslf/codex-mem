...
## Core Principle
**Observe the main Codex Code session from the outside, process observations in the background, inject context at the right time.**
...
┌─────────────────────────────────────────────────────────┐
│              CODEX CODE SESSION                         │
│  (Main session - user interacting with Codex)          │
│                                                          │
...
<Note>
As of Codex Code 2.1.0 (ultrathink update), SessionStart hooks no longer display user-visible messages. Context is silently injected via `hookSpecificOutput.additionalContext`.
</Note>

**Key insight:** Codex-Mem doesn't interrupt or modify Codex Code's behavior. It observes from the outside and provides value through lifecycle hooks.
...
Codex-Mem had several architectural constraints:

1. **Can't modify Codex Code**: It's a closed-source binary
2. **Must be fast**: Can't slow down the main session
3. **Must be reliable**: Can't break Codex Code if it fails
...
Codex Code's hook system provides exactly what we need:
...
Codex-Mem uses lifecycle hook scripts across 5 lifecycle events. Runtime setup is handled out-of-band by `npx codex-mem install` (and `npx codex-mem repair`); the Setup hook only runs a sub-100ms `version-check.js` to flag stale installs. SessionStart runs 2 hooks in sequence: worker-service start, then context-hook.

### Setup Hook: Version Check

**Purpose:** Detect stale installs caused by external plugin upgrades and prompt the user to repair.

**Note:** Runtime installation (Bun, uv, `bun install` in the plugin cache) is performed by `npx codex-mem install` and `npx codex-mem repair` — the Setup hook itself never installs anything.

**When:** Codex Code Setup phase, before every session.

**What it does:**
1. Reads the `.install-version` marker written by the npx installer.
2. Compares it against the currently loaded plugin version.
3. On mismatch, writes `run: npx codex-mem repair` to stderr.
...
```json
{
  "hooks": {
    "Setup": [{
      "hooks": [{
        "type": "command",
        "command": "node ${CODEX_PLUGIN_ROOT}/scripts/version-check.js",
        "timeout": 60
      }]
    }]
  }
...
- ✅ Clear repair instructions on stderr when the plugin was upgraded externally (e.g. `codex plugin update`)

**Source:** `scripts/version-check.js`. The matching installer logic lives in `npx codex-mem install` / `npx codex-mem repair`, which install Bun + uv globally, run `bun install` in the plugin cache, and write the `.install-version` marker — all behind a visible clack spinner.
...
**When:** Codex Code starts (runs after the worker-start SessionStart entry)
...
- ✅ Configurable observation count via `CODEX_MEM_CONTEXT_OBSERVATIONS`
...
**When:** Before Codex processes the user's message
...
```json
{
  "hooks": {
    "UserPromptSubmit": [{
      "hooks": [{
        "type": "command",
        "command": "${CODEX_PLUGIN_ROOT}/scripts/new-hook.js"
      }]
    }]
  }
}
...
**Database operations:**
```sql
INSERT INTO sdk_sessions (codex_session_id, project, user_prompt, ...)
VALUES (?, ?, ?, ...)
...
```json
{
  "hooks": {
    "PostToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "${CODEX_PLUGIN_ROOT}/scripts/save-hook.js"
      }]
    }]
  }
}
...
**When:** When Codex stops (triggered by Stop lifecycle event)

**What it does:**
1. Gathers session observations from database
2. Sends to Codex CLI for summarization
...
```json
{
  "hooks": {
    "Stop": [{
      "hooks": [{
        "type": "command",
        "command": "${CODEX_PLUGIN_ROOT}/scripts/summary-hook.js"
      }]
    }]
  }
}
...
- ✅ Uses Codex CLI for AI compression
...
**When:** Codex Code session ends (not on `/clear`)
...
```json
{
  "hooks": {
    "SessionEnd": [{
      "hooks": [{
        "type": "command",
        "command": "${CODEX_PLUGIN_ROOT}/scripts/cleanup-hook.js"
      }]
    }]
  }
}
...
sequenceDiagram
    ...
    User->>Codex: Start Codex Code
    Codex->>Hooks: SessionStart hook
    Hooks->>DB: Query recent context
    DB-->>Hooks: Session summaries + observations
    Hooks-->>Codex: Inject context
    Note over Codex: Context available for session

    User->>Codex: Submit prompt
    Codex->>Hooks: UserPromptSubmit hook
    ...
    Codex->>Hooks: PostToolUse (multiple times)
...
│  2. Process observation via Codex CLI (5-30s)          │
...
**Technology:** Express.js REST API on the worker's per-user port (default `37700 + (uid % 100)`, override via `CODEX_MEM_WORKER_PORT`)
...
**Principle:** Memory system failure shouldn't break Codex Code
...
```
Without memory: Codex Code works normally
With memory:    Codex Code + context from past sessions
...
[DEBUG] Executing hook command: ${CODEX_PLUGIN_ROOT}/scripts/save-hook.js with timeout 60000ms
...
- All heavy lifting (Bun + uv install, `bun install` inside the plugin cache) happens in `npx codex-mem install` / `npx codex-mem repair`, which run with a visible clack spinner outside the session lifecycle.
- On marker mismatch the hook prints a one-line `run: npx codex-mem repair` hint to stderr and exits 0; the user opts into the slow path explicitly.

### Database Performance

**Schema optimizations:**
- Indexes on `project`, `created_at_epoch`, `codex_session_id`
...
-- Fast: Uses index on codex_session_id
...
**Bottleneck:** Codex API latency (5-30s per observation)
...
3. **Plugin isolation:** `${CODEX_PLUGIN_ROOT}` prevents path traversal
...
**Configuration:**
- Codex API key in `~/.codex/api_key` or `CODEX_API_KEY` env var
- Worker inherits environment from Codex Code
...
3. **Graceful degradation**: Memory system can fail without breaking Codex Code
...
- [Codex Code Hooks Reference](https://docs.codex.com/codex-code/hooks) - Official documentation
...
*The hook-driven architecture enables Codex-Mem to be both powerful and invisible. Users never notice the memory system working - it just makes Codex smarter over time.*
