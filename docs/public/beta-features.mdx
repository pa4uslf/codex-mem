...
1. Open the Codex-Mem viewer (the worker prints its URL on startup)
...
The flagship experimental feature in beta is **Endless Mode** - a biomimetic memory architecture that dramatically extends how long Codex can maintain context in a session.

### The Problem Endless Mode Solves

In standard Codex Code sessions:
...
Worse, Codex **re-synthesizes all previous tool outputs** on every response. This is O(N²) complexity - quadratically growing both in tokens and compute.
...
4. Codex resumes with the compressed context
