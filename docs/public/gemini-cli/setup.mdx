...
description: "Add persistent memory to Gemini CLI with codex-mem"
---
...
Gemini CLI starts every session from scratch. Codex-mem changes that by capturing observations, decisions, and patterns — then injecting relevant context into each new session.

<Info>
**How it works:** Codex-mem installs lifecycle hooks into Gemini CLI that capture tool usage, agent responses, and session events. A local worker service extracts semantic observations and injects relevant history at session start.
</Info>
...
Codex-mem needs an AI provider to extract observations from your sessions. Choose one:
...
<Tabs>
  <Tab title="Gemini API (Free)">
    ...
    cat > ~/.codex-mem/settings.json << 'EOF'
    {
      ...
      "CODEX_MEM_GEMINI_API_KEY": "YOUR_API_KEY"
    }
    EOF
    ```
  ...
  <Tab title="OpenRouter">
    ...
    cat > ~/.codex-mem/settings.json << 'EOF'
    {
      ...
      "CODEX_MEM_OPENROUTER_API_KEY": "YOUR_KEY"
    }
    EOF
    ```
  </Tab>
...
# Check hooks are installed — look for codex-mem entries
cat ~/.gemini/settings.json | grep codex-mem
```
...
Launch Gemini CLI normally. Codex-mem works in the background:

```bash
gemini
```

On session start, you'll see codex-mem context injected with your recent observations and project history.

## What gets captured

Codex-mem registers 8 of Gemini CLI's 11 lifecycle hooks:
...
1. Verify hooks exist in settings:
   ...
   ```
   You should see entries like `"SessionStart"`, `"AfterTool"`, etc. with codex-mem commands.
...
3. Check your AI provider is configured in `~/.codex-mem/settings.json`

### Raw escape codes in output

If you see characters like `[31m` or `[0m` in the session context, your codex-mem version may need updating:
