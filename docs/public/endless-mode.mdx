...
Endless Mode is a **biomimetic memory architecture** that solves Codex's context window exhaustion problem. Instead of keeping full tool outputs in the context window (O(N²) complexity), it:
...
- Maintains two-tier memory: working memory (compressed) + archive memory (full transcript on disk, maintained by default codex code functionality)
...
4. **CodexExecAgent** - Generates observations via Codex CLI, emits completion events
...
```json
{
  "CODEX_MEM_ENDLESS_MODE": "false",  // Default: disabled
  "CODEX_MEM_ENDLESS_WAIT_TIMEOUT_MS": "90000"  // 90 second timeout
}
```
