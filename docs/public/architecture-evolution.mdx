...
**Goal:** Create a memory system that makes Codex smarter across sessions without the user noticing it exists.
...
- High cognitive load for Codex (which tool to use?)
- Code size: ~2,718 lines in mcp-server.ts
```

**The Insight:** Progressive disclosure should be built into tool design itself, not something Codex has to remember.
...
- Native MCP protocol (better Codex integration)
- Works with both Codex Desktop and Codex Code
...
Before: Codex had to remember progressive disclosure pattern
...
- Works identically on Codex Desktop and Codex Code
...
Each layer provides a decision point where Codex can:
- Stop if irrelevant
...
**New idea:** Use Codex itself to compress observations
...
**What we added:**
1. **Codex CLI integration** - Use AI to compress observations
...
- ❌ Multiple SDK sessions per Codex Code session
...
**Problem:** We were creating multiple SDK sessions per Codex Code session.

**What we thought:**
```
Codex Code session → Create SDK session per observation → 100+ SDK sessions
```

**Reality should be:**
```
Codex Code session → ONE long-running SDK session → Streaming input
```
...
┌─────────────────────────────────────────────────────────┐
│              CODEX CODE SESSION                         │
│  User → Codex → Tools (Read, Edit, Write, Bash)        │
│                    ↓                                     │
│              PostToolUse Hook                            │
│              (queues observation)                        │
└─────────────────────────────────────────────────────────┘
...
┌─────────────────────────────────────────────────────────┐
│              SDK WORKER PROCESS                          │
│  ONE streaming session per Codex Code session          │
│                                                          │
...
<Tabs>
  ...
  <Tab title="UserPromptSubmit">
    ...
    **Timing:** Before Codex processes prompt
  ...
  <Tab title="Summary">
    ...
    **What it does:**
    ...
    - Sends to Codex for summarization
...
**Why it broke:**
- Codex Code expects clean JSON or plain text
...
```sql
INSERT INTO sdk_sessions (codex_session_id, user_prompt, ...)
...
The result is a memory system that's both powerful and invisible. Users never notice it working - Codex just gets smarter over time.
