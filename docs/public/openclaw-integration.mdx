...
The OpenClaw plugin gives codex-mem persistent memory to agents running on the [OpenClaw](https://openclaw.ai) gateway. It handles three things:

1. **Observation recording** — Captures tool usage from OpenClaw's embedded runner and sends it to the codex-mem worker for AI processing
...
<Info>
OpenClaw's embedded runner (`pi-embedded`) calls the Codex API directly without spawning a `codex` process, so codex-mem's standard hooks never fire. This plugin bridges that gap by using OpenClaw's event system to capture the same data.
</Info>
...
The observation feed sends a formatted message to your OpenClaw channel every time codex-mem creates a new observation. Each message includes the observation title and subtitle so you can follow along as your agents work.
...
Add the `observationFeed` block to your codex-mem plugin config in your OpenClaw gateway configuration:
...
2. **Use the status command** — Run `/codex_mem_feed` in any OpenClaw chat to see:
   ```
...
# Fully unattended (defaults to Codex Max Plan)
...
Add `codex-mem` to your OpenClaw gateway's plugin configuration:
...
<Note>
The codex-mem worker service must be running on the same machine as the OpenClaw gateway. The plugin communicates with it via HTTP on `localhost:37777`.
</Note>
...
<ParamField body="workerPort" type="number" default={37777}>
  Port for the codex-mem worker service. Override if your worker runs on a non-default port.
</ParamField>
...
```
/codex_mem_feed        # Show current status
/codex_mem_feed on     # Request enable
/codex_mem_feed off    # Request disable
```
...
The plugin uses HTTP calls to the already-running codex-mem worker service rather than spawning subprocesses. This means:

- No `bun` dependency required on the gateway
- No process spawn overhead per event
- Uses the same worker API that Codex Code hooks use
- All operations are non-blocking (fire-and-forget where possible)

### Session Tracking

Each OpenClaw agent session gets a unique `contentSessionId` (format: `openclaw-<sessionKey>-<timestamp>`) that maps to a codex-mem session in the worker. The plugin tracks:
...
- Codex-mem worker service running on `localhost:37777` (or configured port)
