...
Knowledge agents let you compile a slice of your codex-mem observation history into a **queryable "brain"** that answers questions conversationally. Instead of getting raw search results back, you get synthesized, grounded answers drawn from your actual project history -- decisions, discoveries, bugfixes, and features.
...
```
build_corpus name="hooks-expertise" query="hooks architecture" project="codex-mem" limit=200
```

This searches your observation history, collects matching records, and saves them as a corpus file. Then prime it — this loads the corpus into a Codex session's context window:

```
prime_corpus name="hooks-expertise"
```

Your knowledge agent is ready. The returned `session_id` **is** the agent — a Codex session with your history baked in.
...
Under the hood, `CorpusBuilder` searches your observations, hydrates full records, parses structured fields (facts, concepts, files), calculates stats, and writes everything to `~/.codex-mem/corpora/hooks-expertise.corpus.json`.

### 2. Prime the Knowledge Agent

Priming loads the entire corpus into a Codex session's context window.

```bash
curl -X POST http://localhost:37777/api/corpus/hooks-expertise/prime
```

The agent renders all observations into full-detail text and feeds them to the Codex CLI. Codex reads the corpus and acknowledges the themes. The returned `session_id` **is** the knowledge agent -- a Codex session with your history baked in.
...
reprime_corpus ──┘                          |
                              ...
                              CorpusBuilder  |  KnowledgeAgent
                                  |          |       |
                        SearchOrchestrator   |   Codex CLI V1
...
- **SDK process exit**: If the Codex process exits after yielding all messages, the agent treats it as success when the session_id or answer was already captured
- **Empty corpus**: A corpus with 0 observations is valid (just empty)
- **Model from settings**: Reads `CODEX_MEM_MODEL` from user settings -- no hardcoded model IDs
