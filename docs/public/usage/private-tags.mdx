...
Codex can see and use this content during the current session, but it won't be saved as an observation.
...
Codex sees the full error but only the question gets stored.
...
**Important**: Tags are stripped during storage, not from the live conversation. Codex sees the full content including `<private>` tags during the session, and they only disappear when content is persisted to the database.

### What Doesn't Get Filtered

- Session summaries (generated from non-private observations only)
- Codex's responses (not captured by codex-mem)
...
The API key won't be stored, but Codex can use it during the session.
...
The personal context helps Codex understand your request without polluting your observation history.

## Best Practices

1. **Don't over-tag**: Only use `<private>` for content you genuinely don't want stored
2. **Context matters**: Codex's understanding of your project comes from observations - excessive private tagging reduces future context quality
