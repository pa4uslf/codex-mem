...
|---------|--------|---------|-------------|
| `CODEX_MEM_PROVIDER` | `codex`, `gemini`, `openrouter` | `codex` | AI provider for observation extraction |
| `CODEX_MEM_OPENROUTER_API_KEY` | string | — | Your OpenRouter API key |
| `CODEX_MEM_OPENROUTER_MODEL` | string | `xiaomi/mimo-v2-flash:free` | Model identifier (see list above) |
| `CODEX_MEM_OPENROUTER_MAX_CONTEXT_MESSAGES` | number | `20` | Max messages in conversation history |
| `CODEX_MEM_OPENROUTER_MAX_TOKENS` | number | `100000` | Token budget safety limit |
| `CODEX_MEM_OPENROUTER_SITE_URL` | string | — | Optional: URL for analytics attribution |
| `CODEX_MEM_OPENROUTER_APP_NAME` | string | `codex-mem` | Optional: App name for analytics |
...
```json
{
  ...
  "CODEX_MEM_OPENROUTER_API_KEY": "sk-or-v1-your-key-here",
...
|-------|----------------------|----------|
| `openai/gpt-5` | $3 in / $15 out | Highest quality observations |
...
Either:
- Set `CODEX_MEM_OPENROUTER_API_KEY` in `~/.codex-mem/settings.json`, or
...
- Reduce `CODEX_MEM_OPENROUTER_MAX_CONTEXT_MESSAGES`
- Reduce `CODEX_MEM_OPENROUTER_MAX_TOKENS`
...
| Feature | Codex (SDK) | Gemini | OpenRouter |
|---------|-------------|--------|------------|
| **Cost** | Pay per token | Free tier + paid | Free models + paid |
| **Models** | Codex only | Gemini only | 100+ models |
...
<Tip>
**Recommendation**: Start with OpenRouter's free `xiaomi/mimo-v2-flash:free` model for zero-cost observation extraction. If you need higher quality or encounter rate limits, switch to Codex or add OpenRouter credits for premium models.
</Tip>
