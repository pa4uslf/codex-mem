...
1. **Start Codex Code** - Context from last 10 sessions appears automatically
2. **Work normally** - Every tool execution is captured
3. **Codex finishes responding** - Stop hook automatically generates and saves a summary
...
- And all other Codex Code tools
...
When Codex finishes responding (triggering the Stop hook), a summary is automatically generated with:

- **Request** - What you asked for
- **Investigated** - What Codex explored
...
When you start a new Codex Code session, the SessionStart hook:
...
5. Injects formatted context into Codex's initial context
...
- Codex auto-invokes MCP search tools to fetch full details
...
- ✅ **A summary will be generated** when Codex finishes responding (Stop hook fires)

The `/clear` command clears the conversation context visible to Codex AND re-injects fresh context from recent sessions, while the underlying session continues tracking observations.
...
Codex automatically recognizes your intent and invokes the MCP search tools, which use a 3-layer workflow (search → timeline → get_observations) for efficient token usage.
