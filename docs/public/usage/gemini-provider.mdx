...
description: "Use Google's Gemini API as an alternative to Codex for observation extraction"
---

# Gemini Provider

Codex-mem supports Google's Gemini API as an alternative to the Codex CLI for extracting observations from your sessions. This can significantly reduce costs since Gemini offers a generous free tier.
...
- **Same quality**: Gemini extracts observations using the same XML format as Codex
...
|---------|--------|---------|-------------|
| `CODEX_MEM_PROVIDER` | `codex`, `gemini` | `codex` | AI provider for observation extraction |
| `CODEX_MEM_GEMINI_API_KEY` | string | — | Your Gemini API key |
| `CODEX_MEM_GEMINI_MODEL` | `gemini-2.5-flash-lite`, `gemini-2.5-flash`, `gemini-3-flash-preview` | `gemini-2.5-flash-lite` | Gemini model to use |
| `CODEX_MEM_GEMINI_BILLING_ENABLED` | `true`, `false` | `false` | Skip rate limiting if billing is enabled on Google Cloud |
...
```json
{
  ...
  "CODEX_MEM_GEMINI_API_KEY": "your-api-key-here",
  "CODEX_MEM_GEMINI_MODEL": "gemini-2.5-flash-lite",
  "CODEX_MEM_GEMINI_BILLING_ENABLED": "true"
}
```
...
You can switch between Codex and Gemini at any time:
...
If Gemini is selected and the API errors, codex-mem logs the failure and re-throws so the message stays pending for later retry. There is no Codex CLI fallback — earlier docs claimed automatic Codex fallback, but the wiring was never actually engaged in production (#2087). To switch providers, change `CODEX_MEM_PROVIDER` in settings.
...
Either:
- Set `CODEX_MEM_GEMINI_API_KEY` in `~/.codex-mem/settings.json`, or
...
Codex-mem enforces these limits automatically with built-in delays between requests. Processing may be slower but stays within limits.
...
- Codex-mem automatically falls back to Codex CLI
- Or switch back to Codex as your primary provider

### Observation Quality

If observations seem lower quality with Gemini:
- Note that Codex typically produces slightly higher quality observations
- Consider using Gemini for cost savings and Codex for important projects
