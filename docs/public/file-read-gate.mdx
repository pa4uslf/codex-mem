...
description: "How codex-mem intercepts file reads to save tokens using observation history"
---
...
The File Read Gate is a **PreToolUse hook** that intercepts Codex's `Read` tool calls. When Codex tries to read a file that has prior observations in the database, the gate blocks the read and instead shows a compact timeline of past work on that file. Codex then decides the cheapest path to get the context it needs.
...
```
Codex calls Read("src/services/worker-service.ts")
         ↓
...
When the gate fires, Codex sees a message like this:
...
Codex has four options after seeing the timeline, ordered from cheapest to most expensive:

| Option | Token Cost | When to Use |
|--------|-----------|-------------|
| **Semantic priming** | 0 extra | Timeline titles tell Codex enough to proceed |
...
This lets Codex reason about how recent the observations are relative to now. For example:
...
The timestamp format matches the session start context header (`YYYY-MM-DD time timezone`), so Codex sees consistent temporal markers throughout its session.
...
If Codex needs more detail, it fetches individual observations at ~300 tokens each. Even fetching 3 observations totals ~1,270 tokens -- still a **75-97% savings** over reading the full file.
...
Projects matching patterns in `CODEX_MEM_EXCLUDED_PROJECTS` skip the gate entirely. Configure this in `~/.codex-mem/settings.json`:

```json
{
  "CODEX_MEM_EXCLUDED_PROJECTS": "/tmp/*,/scratch/*"
}
```
...
1. Open your Codex Code settings:
   ```
   ~/.codex/settings.json
   ```

2. Find the codex-mem hooks section under `hooks.PreToolUse` and remove the entry with the `Read` matcher.

Alternatively, if you want to keep the gate installed but bypass it for a specific read, Codex can ask you to allow the read -- the gate's deny decision is presented to the user, who can override it.

<Note>
Disabling the gate means Codex will read full files every time, which increases token usage but ensures it always sees the latest code. This is a reasonable choice for small projects or when observations are sparse.
</Note>
...
The File Read Gate is one piece of codex-mem's layered context strategy:
...
Each layer is progressively more expensive. The gate ensures Codex starts at the cheapest layer and escalates only when needed.
