...
// or whatever the user set via CODEX_MEM_WORKER_PORT / settings.json. Read it from
// process.env.CODEX_MEM_WORKER_PORT, then ~/.codex-mem/settings.json
// (CODEX_MEM_WORKER_PORT key), then fall back to the deterministic default.
...
Body: { codexSessionId, tool_name, tool_input, tool_response, cwd }

// Queue summary
POST /api/sessions/summarize
Body: { codexSessionId, last_user_message, last_assistant_message }
...
```bash
CODEX_MEM_MODEL=gpt-5-mini  # Default Codex model for observations/summaries
CODEX_MEM_CONTEXT_OBSERVATIONS=50          # Observations injected at SessionStart
CODEX_MEM_WORKER_PORT=                     # Optional override; default = 37700 + (uid % 100)
CODEX_MEM_DATA_DIR=                        # Optional override for the data directory
...
npm run sync-marketplace      # Copy to ~/.codex/plugins
...
<CardGroup cols={2}>
  ...
  <Card title="SDKAgent" icon="robot">
    Codex CLI for generating observations/summaries
  </Card>
...
{
  "codexSessionId": "abc123",      // Codex session identifier (string)
  "tool_name": "Bash",
...
<Note>
New integrations should use `/api/sessions/*` endpoints with `codexSessionId`.
</Note>
...
- `new_prompt`: { type, id, codex_session_id, project, prompt_number, prompt_text, created_at_epoch }
</Info>
...
interface ActiveSession {
  ...
  codexSessionId: string;              // Codex session identifier (string)
...
### Mapping Codex Code Hooks to Worker API
...
const participant = vscode.chat.createChatParticipant('codex-mem', async (request, context, stream, token) => {
  const codexSessionId = context.session.id;
...
class WorkerUnavailableError extends Error {
  constructor() {
    super('Codex-mem worker is not running or unreachable');
...
describe('Worker Integration', () => {
  ...
  beforeAll(async () => {
    ...
    workerProcess = spawn('node', ['dist/worker-service.js'], {
      env: { ...process.env, CODEX_MEM_WORKER_PORT: '37778' }
    });
...
export class WorkerClient {
  ...
  async completeSession(codexSessionId: string): Promise<void> {
...
<Warning>
### sessionDbId vs codexSessionId

**IMPORTANT:** Use `codexSessionId` (string) for new API endpoints, not `sessionDbId` (number).

- `sessionDbId` - Numeric database ID (legacy endpoints only)
- `codexSessionId` - String identifier from Codex platform (new endpoints)
</Warning>
