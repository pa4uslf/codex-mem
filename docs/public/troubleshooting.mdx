...
Describe any issues you're experiencing to Codex, and the troubleshoot skill will automatically activate to provide diagnosis and fixes.
...
1. Find the worker port. The default is `37700 + (uid % 100)`. The configured port is the value of `CODEX_MEM_WORKER_PORT` in `~/.codex-mem/settings.json`; the running worker also reports it on `/api/health`:
   ```bash
   PORT=$(jq -r .CODEX_MEM_WORKER_PORT ~/.codex-mem/settings.json)
   lsof -i :$PORT
...
1. Clear install cache:
   ```bash
   rm ~/.codex/plugins/marketplaces/thedotmack/.install-version
   ```
...
3. Check version marker:
   ```bash
   cat ~/.codex/plugins/marketplaces/thedotmack/.install-version
   cat ~/.codex/plugins/marketplaces/thedotmack/package.json | grep version
   ```

4. Restart Codex Code after manual install
...
CREATE TABLE pending_messages (
  ...
  codex_session_id TEXT,          -- Codex session ID
...
**Symptoms**: No session context when Codex starts.
...
**Symptoms**: Hook execution times out, errors in Codex Code.
...
3. Check package.json exists:
   ```bash
   ls -la ~/.codex/plugins/marketplaces/thedotmack/package.json
   ```
...
**Symptoms**: MCP search tools not visible in Codex Code.
...
3. Verify installation:
   ```bash
   ls -la ~/.codex/plugins/marketplaces/thedotmack/
   ```
...
jq -r .CODEX_MEM_WORKER_PORT ~/.codex-mem/settings.json
