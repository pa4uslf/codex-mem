...
<h4 align="center"><a href="https://codex.com/codex-code" target="_blank">Codex Code</a>向けに構築された永続的メモリ圧縮システム</h4>
...
<p align="center">
  Codex-Memは、ツール使用の観察を自動的にキャプチャし、セマンティックサマリーを生成して将来のセッションで利用可能にすることで、セッション間のコンテキストをシームレスに保持します。これにより、Codexはセッションが終了または再接続された後でも、プロジェクトに関する知識の連続性を維持できます。
</p>
...
ターミナルで新しいCodex Codeセッションを開始し、次のコマンドを入力します:
...
Codex Codeを再起動します。以前のセッションからのコンテキストが新しいセッションに自動的に表示されます。
...
- 💻 **Codex Desktopスキル** - Codex Desktopの会話からメモリを検索
...
📚 **[完全なドキュメントを見る](https://docs.codex-mem.ai/)** - 公式ウェブサイトで閲覧
...
- Codexは自動的にmem-searchスキルを呼び出して関連するコンテキストを検索します
...
- **Node.js**: 18.0.0以上
- **Codex Code**: プラグインサポートを備えた最新バージョン
...
問題が発生した場合は、Codexに問題を説明すると、troubleshootスキルが自動的に診断して修正を提供します。
...
**Codex Agent SDKで構築** | **Codex Codeで動作** | **TypeScriptで作成**
