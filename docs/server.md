...
Codex-Mem Server is the beta server runtime for Codex-Mem 13. It is a
Postgres-backed, BullMQ-driven, API-key-authenticated runtime that replaces
the legacy `codex-mem worker` for deployable use cases.
...
2. Generation can scale horizontally (`docker compose up --scale codex-mem-worker=N`).
3. Restarting the HTTP server does not lose enqueued generation work — jobs
   live in Valkey, persisted by AOF.

The legacy `codex-mem worker` runtime is **not** spawned in Docker. The
...
|-----------------------------------|----------|--------------------------------------------------------------|
| `CODEX_MEM_RUNTIME`              | Docker   | Must be `server-beta` in Docker (warned otherwise).          |
| `CODEX_MEM_QUEUE_ENGINE`         | Docker   | Must be `bullmq`. In-process queues are rejected in Docker.  |
| `CODEX_MEM_SERVER_DATABASE_URL`  | Always   | Postgres connection string. Fails fast at startup.           |
| `CODEX_MEM_REDIS_URL`            | bullmq   | Required when queue engine is `bullmq`.                      |
| `CODEX_MEM_AUTH_MODE`            | Always   | Must NOT be `local-dev` in Docker.                           |
| `CODEX_MEM_ALLOW_LOCAL_DEV_BYPASS` | Docker | Must NOT be `1`/`true` in Docker.                            |
| `CODEX_MEM_GENERATION_DISABLED`  | Optional | Set to `true` on the HTTP service when running a separate worker. |
| `CODEX_MEM_SERVER_PROVIDER`      | Worker   | One of `codex`, `gemini`, `openrouter`. Worker only.        |
| `CODEX_API_KEY` (or alt)      | Worker   | Required by the chosen provider.                             |

Local development can still use SQLite + `local-dev` auth bypass **outside
Docker only**. Deployable mode must use the table above.

## Generation worker mode (`codex-mem server worker start`)
...
* Forces generation enabled even if `CODEX_MEM_GENERATION_DISABLED=true`
  is inherited from the shared compose file. The worker IS the generation
  process.

In Compose this is the `codex-mem-worker` service. Scale it horizontally:

```sh
docker compose up -d --scale codex-mem-worker=4
```
...
> **Do not enable `CODEX_MEM_AUTH_MODE=local-dev` in Docker.** The
...
* `codex-mem-server` — HTTP runtime.
  `CODEX_MEM_GENERATION_DISABLED=true` so the BullMQ Worker is **not**
  attached here.
* `codex-mem-worker` — generation worker. Scale horizontally.
...
* Restart of `codex-mem-server` and `codex-mem-worker` mid-stream does
  not lose data.
* Revoking an API key denies subsequent reads and writes (401/403).
* No `worker-service.cjs` process runs in any container.
* `CODEX_MEM_AUTH_MODE=local-dev` is rejected inside Docker.
