...
#
# Brings up Postgres + Valkey + codex-mem-server (HTTP) + codex-mem-worker
...
PROJECT_NAME="${COMPOSE_PROJECT_NAME:-codex-mem-server-beta-e2e-$(date +%s)}"
...
export POSTGRES_USER="${POSTGRES_USER:-codexmem_e2e}"
export POSTGRES_PASSWORD="${POSTGRES_PASSWORD:-codexmem_e2e}"
export POSTGRES_DB="${POSTGRES_DB:-codexmem_e2e}"
COMPOSE=(docker compose -p "$PROJECT_NAME" "${COMPOSE_FILES[@]}")
SERVER_SCRIPT="/opt/codex-mem/scripts/server-beta-service.cjs"
...
cleanup() {
  ...
  if [[ $exit_code -ne 0 ]]; then
    ...
    "${COMPOSE[@]}" logs --no-color --tail=200 \
      codex-mem-server codex-mem-worker valkey postgres >&2 || true
  fi
...
wait_for_container_readiness() {
  ...
  until "${COMPOSE[@]}" exec -T codex-mem-server curl -fsS "$SERVER_HEALTH_URL" >/dev/null 2>&1; do
...
create_key() {
  local name="$1"
  local scopes="$2"
  "${COMPOSE[@]}" exec -T codex-mem-server \
...
assert_no_worker_process() {
  ...
  # The new generation worker is named `codex-mem-worker`; that's allowed.
  local services
  services="$("${COMPOSE[@]}" ps --services)"
  if echo "$services" | grep -E '(^|\s)worker($|\s)' | grep -v 'codex-mem-worker' >/dev/null; then
  ...
  if "${COMPOSE[@]}" exec -T codex-mem-server pgrep -af 'worker-service\.cjs' >/dev/null 2>&1; then
    echo "[e2e] FAIL — worker-service.cjs is running inside codex-mem-server" >&2
    "${COMPOSE[@]}" exec -T codex-mem-server pgrep -af 'worker-service\.cjs' >&2 || true
    return 1
  fi
  if "${COMPOSE[@]}" exec -T codex-mem-worker pgrep -af 'worker-service\.cjs' >/dev/null 2>&1; then
    echo "[e2e] FAIL — worker-service.cjs is running inside codex-mem-worker" >&2
    return 1
  fi
...
assert_local_dev_rejected_in_docker() {
  ...
  # Run a throwaway server-beta container with CODEX_MEM_AUTH_MODE=local-dev.
  # validateServerBetaEnv() should refuse to start and exit non-zero.
  local rc=0
  "${COMPOSE[@]}" run --rm \
    --no-deps \
    -e CODEX_MEM_AUTH_MODE=local-dev \
    -e CODEX_MEM_ALLOW_LOCAL_DEV_BYPASS=1 \
    -e CODEX_MEM_CONTAINER_MODE=server \
    codex-mem-server >/tmp/local-dev-stdout.$$ 2>/tmp/local-dev-stderr.$$ \
    || rc=$?
  if [[ $rc -eq 0 ]]; then
    echo "[e2e] FAIL — server-beta started with CODEX_MEM_AUTH_MODE=local-dev in Docker" >&2
...
"${COMPOSE[@]}" up --build -d postgres valkey codex-mem-server codex-mem-worker
wait_for_container_readiness
assert_no_worker_process
...
"${COMPOSE[@]}" restart codex-mem-server codex-mem-worker
wait_for_container_readiness
assert_no_worker_process
