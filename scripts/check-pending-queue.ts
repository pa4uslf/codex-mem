...
function resolveWorkerPort(): number {
  const raw = process.env.CODEX_MEM_WORKER_PORT;
  if (raw === undefined || raw === '') return DEFAULT_WORKER_PORT;
  const parsed = parseInt(raw, 10);
  if (!Number.isInteger(parsed) || parsed < 1 || parsed > 65535) {
    console.warn(
      `[check-pending-queue] Invalid CODEX_MEM_WORKER_PORT=${JSON.stringify(raw)}; ` +
...
async function main() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`
Codex-Mem Pending Queue Manager
...
What is this for?
  If the codex-mem worker has unprocessed observations queued, this script
...
`);
  ...
  console.log('\n=== Codex-Mem Pending Queue Status ===\n');

  const healthy = await checkWorkerHealth();
  if (!healthy) {
    console.log(`Worker is not running at ${WORKER_URL}. Start it with:`);
    console.log('  cd ~/.codex/plugins/marketplaces/thedotmack && npm run worker:start\n');
    process.exit(1);
  }
