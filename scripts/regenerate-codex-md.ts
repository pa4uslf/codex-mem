...
const DB_PATH = path.join(os.homedir(), '.codex-mem', 'codex-mem.db');
const SETTINGS_PATH = path.join(os.homedir(), '.codex-mem', 'settings.json');
const settings = SettingsDefaultsManager.loadFromFile(SETTINGS_PATH);
const OBSERVATION_LIMIT = parseInt(settings.CODEX_MEM_CONTEXT_OBSERVATIONS, 10) || 50;
...
import { replaceTaggedContent } from '../src/utils/codex-md-utils.js';
...
function writeCodexMdToFolderForRegenerate(folderPath: string, newContent: string): void {
...
function cleanupAutoGeneratedFiles(workingDir: string, dryRun: boolean): void {
  console.log('=== CODEX.md Cleanup Mode ===\n');
  console.log(`Scanning ${workingDir} for CODEX.md files with auto-generated content...\n`);
  ...
  if (filesToProcess.length === 0) {
    console.log('No CODEX.md files with auto-generated content found.');
    return;
  }

  console.log(`Found ${filesToProcess.length} CODEX.md files with auto-generated content:\n`);
...
function regenerateFolder(
  ...
  try {
    ...
    writeCodexMdToFolderForRegenerate(absoluteFolder, formatted);
...
async function main() {
  ...
  console.log('=== CODEX.md Regeneration Script ===\n');
