...
function parseWorkerPort(rawPort: unknown): number {
  if (typeof rawPort !== 'string' || rawPort.trim() === '') {
    throw new Error('Invalid CODEX_MEM_WORKER_PORT in settings.json: missing');
  }

  const normalized = rawPort.trim();
  const port = Number.parseInt(normalized, 10);
  if (!Number.isInteger(port) || port < 1 || port > 65535 || String(port) !== normalized) {
    throw new Error(`Invalid CODEX_MEM_WORKER_PORT in settings.json: ${rawPort}`);
  }
  return port;
}
...
export async function exportMemories(query: string, outputFile: string, project?: string) {
  ...
  const port = parseWorkerPort(settings.CODEX_MEM_WORKER_PORT);
...
function isDirectRun(): boolean {
  if (process.env.CODEX_MEM_EXPORT_MEMORIES_NO_MAIN === '1') {
    return false;
  }
...
if (isDirectRun()) {
  ...
  if (args.length < 2) {
    ...
    console.error('Example: npx tsx scripts/export-memories.ts "windows" windows-memories.json --project=codex-mem');
