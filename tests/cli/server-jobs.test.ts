...
// Phase 12 — `codex-mem server jobs` operator console. Uses the
...
describe('Phase 12 — server jobs CLI', () => {
  ...
  beforeEach(() => {
    ...
    process.env.CODEX_MEM_SERVER_DATABASE_URL = 'postgres://test/test';
    process.env.CODEX_MEM_SERVER_ADMIN = '1';
  });
  ...
  it('refuses unscoped operations without admin override', async () => {
    delete process.env.CODEX_MEM_SERVER_ADMIN;
