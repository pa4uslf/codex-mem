...
describe('server API key auth', () => {
  ...
  it('middleware blocks local-dev bypass when forwarded proxy headers are present', () => {
    ...
    const req: any = {
      ...
      header: (name: string) => {
        ...
        if (normalized === 'host') return 'codex-mem.example.com';
  ...
  it('middleware defaults to API-key auth when auth mode is not explicitly set', () => {
    const originalAuthMode = process.env.CODEX_MEM_AUTH_MODE;
    delete process.env.CODEX_MEM_AUTH_MODE;
    try {
      ...
      if (originalAuthMode === undefined) {
        ...
        process.env.CODEX_MEM_AUTH_MODE = originalAuthMode;
      }
    }
  });
