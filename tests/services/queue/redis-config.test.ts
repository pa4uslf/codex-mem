...
describe('redis queue config', () => {
  ...
  test('loads queue settings from settings file with env override precedence', async () => {
    tempDir = mkdtempSync(join(tmpdir(), 'codex-mem-redis-config-'));
    const settingsPath = join(tempDir, 'settings.json');
    writeFileSync(settingsPath, JSON.stringify({
      CODEX_MEM_QUEUE_ENGINE: 'bullmq',
      CODEX_MEM_REDIS_MODE: 'external',
      CODEX_MEM_REDIS_HOST: 'settings-host',
      CODEX_MEM_REDIS_PORT: '6381',
      CODEX_MEM_REDIS_URL: '',
      CODEX_MEM_QUEUE_REDIS_PREFIX: 'settings-prefix',
    }), 'utf-8');
    ...
    setEnv('CODEX_MEM_REDIS_HOST', 'env-host');
