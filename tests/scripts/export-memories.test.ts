...
const originalDataDir = process.env.CODEX_MEM_DATA_DIR;
const originalNoMain = process.env.CODEX_MEM_EXPORT_MEMORIES_NO_MAIN;
...
describe('export-memories script', () => {
  ...
  afterEach(() => {
    ...
    if (originalDataDir === undefined) {
      ...
      process.env.CODEX_MEM_DATA_DIR = originalDataDir;
    }

    if (originalNoMain === undefined) {
      delete process.env.CODEX_MEM_EXPORT_MEMORIES_NO_MAIN;
    } else {
      process.env.CODEX_MEM_EXPORT_MEMORIES_NO_MAIN = originalNoMain;
    }
  ...
  it('loads settings from CODEX_MEM_DATA_DIR and sends canonical memorySessionIds', async () => {
    tempDir = mkdtempSync(join(tmpdir(), 'codex-mem-export-'));
    process.env.CODEX_MEM_DATA_DIR = tempDir;
    process.env.CODEX_MEM_EXPORT_MEMORIES_NO_MAIN = '1';
    writeFileSync(join(tempDir, 'settings.json'), JSON.stringify({
      CODEX_MEM_WORKER_PORT: '45678',
    }));
  ...
  it('rejects an invalid worker port before fetching', async () => {
    ...
    process.env.CODEX_MEM_EXPORT_MEMORIES_NO_MAIN = '1';
    writeFileSync(join(tempDir, 'settings.json'), JSON.stringify({
      CODEX_MEM_WORKER_PORT: '45678abc',
    }));
  ...
  it('rejects an empty worker port with a clear configuration error', async () => {
    ...
    await expect(exportMemories('needle', join(tempDir, 'export.json'))).rejects.toThrow(
      'Invalid CODEX_MEM_WORKER_PORT in settings.json: missing',
    );
    expect(fetchMock).not.toHaveBeenCalled();
  });
