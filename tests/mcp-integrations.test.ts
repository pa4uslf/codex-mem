...
describe('MCP Integrations', () => {
  ...
  describe('writeMcpJsonConfig', () => {
    ...
    it('writes valid JSON with codex-mem entry', () => {
      ...
      expect(config.mcpServers['codex-mem'].command).toBe(process.execPath);
      expect(config.mcpServers['codex-mem'].args).toEqual(['/path/to/mcp.cjs']);
    });
    ...
    it('uses custom serversKeyName when provided', () => {
      ...
      expect(config.servers['codex-mem']).toBeDefined();
      expect(config.mcpServers).toBeUndefined();
    });

    it('preserves existing servers when adding codex-mem', () => {
      const configPath = join(tempDir, 'mcp.json');
  ...
  describe('idempotency', () => {
    it('running install twice does not create duplicate entries', () => {
      ...
      const codexMemEntries = serverKeys.filter((k) => k === 'codex-mem');
      expect(codexMemEntries).toHaveLength(1);
    });
    ...
    it('updates the server path on re-install', () => {
      ...
      const config = JSON.parse(readFileSync(configPath, 'utf-8'));
      expect(config.mcpServers['codex-mem'].args).toEqual(['/new/path/mcp.cjs']);
    });
  });
