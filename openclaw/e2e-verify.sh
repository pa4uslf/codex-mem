...
if echo "$PLUGIN_LIST" | grep -q "codex-mem"; then
...
PLUGIN_INFO=$(node /app/openclaw.mjs plugins info codex-mem 2>&1 || true)
if echo "$PLUGIN_INFO" | grep -qi "codex-mem"; then
  pass "Plugin info shows codex-mem details"
else
  fail "Plugin info failed"
  echo "$PLUGIN_INFO"
fi

if echo "$PLUGIN_LIST" | grep -A1 "codex-mem" | grep -qi "enabled\|loaded"; then
...
if [ ! -d "$EXTENSIONS_DIR" ]; then
  EXTENSIONS_DIR="/home/node/.openclaw/extensions/codex-mem"
  if [ ! -d "$EXTENSIONS_DIR" ]; then
...
echo "  Starting mock codex-mem worker..."
...
if grep -qi "codex-mem" "$GATEWAY_LOG" 2>/dev/null; then
  pass "Gateway log mentions codex-mem plugin"
else
  fail "Gateway log does not mention codex-mem"
