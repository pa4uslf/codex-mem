/**
 * Shared Codex executable discovery and validation.
 *
 * Used by SDKAgent and KnowledgeAgent to locate a working Codex Code CLI.
 ...
 * on Windows via a crafted CODEX_CODE_PATH in settings.json.
 */
function verifyCodexVersion(candidate: string): string | null {
  try {
...
/**
 * Find and validate a Codex Code CLI executable.
 *
 * Discovery order:
 *   1. `CODEX_CODE_PATH` from settings.json (explicit user override)
 *   2. `codex.cmd` via PATH on Windows (avoids spawn issues with spaces)
 *   3. `which codex` / `where codex` auto-detection
 *
 ...
 * @throws {Error} when no valid Codex CLI can be found
 */
export function findCodexExecutable(logComponent: Component = 'SDK'): string {
  const settings = SettingsDefaultsManager.loadFromFile(USER_SETTINGS_PATH);

  // --- 1. Explicit configured path ----------------------------------------
  if (settings.CODEX_CODE_PATH) {
    if (!existsSync(settings.CODEX_CODE_PATH)) {
      throw new Error(
        `CODEX_CODE_PATH is set to "${settings.CODEX_CODE_PATH}" but the file does not exist.`
      );
    }

    const version = verifyCodexVersion(settings.CODEX_CODE_PATH);
    if (!version) {
      const isDesktopApp = looksLikeDesktopAppPath(settings.CODEX_CODE_PATH);
      if (isDesktopApp) {
        throw new Error(
          `Found desktop app at "${settings.CODEX_CODE_PATH}" but it doesn't support headless mode. ` +
          `Install Codex CLI from OpenAI, then ensure the codex command is on PATH.`
        );
      }
      throw new Error(
        `CODEX_CODE_PATH is set to "${settings.CODEX_CODE_PATH}" but it failed the --version check. ` +
        `Ensure this is a working Codex Code CLI binary.`
      );
    }
    logger.debug(logComponent, `Using configured CODEX_CODE_PATH: ${settings.CODEX_CODE_PATH} (${version})`);
    return settings.CODEX_CODE_PATH;
  }

  // --- 2. Windows: prefer codex.cmd via PATH ------------------------------
  if (process.platform === 'win32') {
    try {
      execSync('where codex.cmd', {
        encoding: 'utf8',
        windowsHide: true,
        stdio: ['ignore', 'pipe', 'ignore'],
      });
      // codex.cmd is a wrapper — verify it can actually produce --version
      const version = verifyCodexVersion('codex.cmd');
      if (version) {
        logger.debug(logComponent, `Using codex.cmd from PATH (${version})`);
        return 'codex.cmd';
      }
      logger.warn(logComponent, 'codex.cmd found in PATH but failed --version check, trying next candidate');
    } catch {
  ...
  try {
    ...
    for (const candidate of candidates) {
      const version = verifyCodexVersion(candidate);
      if (version) {
        logger.debug(logComponent, `Auto-detected Codex CLI: ${candidate} (${version})`);
        return candidate;
      }
    ...
    if (error instanceof Error) {
      logger.debug(logComponent, 'Codex executable auto-detection failed', {}, error);
    } else {
      logger.debug(logComponent, 'Codex executable auto-detection failed with non-Error', {}, new Error(String(error)));
    }
  }

  throw new Error(
    'Codex executable not found. Please either:\n' +
    '1. Add "codex" to your system PATH, or\n' +
    '2. Set CODEX_CODE_PATH in ~/.codex-mem/settings.json'
  );
}
