...
function getTrackedFolders(workingDir: string): Set<string> {
  ...
  try {
    ...
    logger.warn('CODEX_MD', 'git ls-files failed, falling back to directory walk', { error: errorMessage });
...
function hasDirectChildFile(obs: ObservationRow, folderPath: string): boolean {
  const checkFiles = (filesJson: string | null): boolean => {
    ...
    try {
      ...
      logger.warn('CODEX_MD', 'Failed to parse files JSON in hasDirectChildFile', { error: error instanceof Error ? error.message : String(error) });
    }
    return false;
  };
...
function extractRelevantFile(obs: ObservationRow, relativeFolder: string): string {
  if (obs.files_modified) {
    try {
      ...
      logger.warn('CODEX_MD', 'Failed to parse files_modified JSON', { error: error instanceof Error ? error.message : String(error) });
    }
  }
  ...
  if (obs.files_read) {
    try {
      ...
      logger.warn('CODEX_MD', 'Failed to parse files_read JSON', { error: error instanceof Error ? error.message : String(error) });
    }
  }
...
function formatObservationsForCodexMd(observations: ObservationRow[], folderPath: string): string {
  const lines: string[] = [];
...
function writeCodexMdToFolder(folderPath: string, newContent: string): void {
  ...
  const codexMdPath = path.join(folderPath, 'CODEX.md');
...
function regenerateFolder(
  ...
  try {
    const formatted = formatObservationsForCodexMd(observations, relativeFolder);
    writeCodexMdToFolder(absoluteFolder, formatted);
    return { success: true, observationCount: observations.length };
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger.warn('CODEX_MD', 'Failed to regenerate folder', { folder: relativeFolder, error: errorMessage });
...
function processAllFoldersForGeneration(
  ...
  for (const absoluteFolder of foldersArray) {
    ...
    if (result.success) {
      logger.debug('CODEX_MD', `Processed folder: ${relativeFolder}`, {
      ...
      logger.warn('CODEX_MD', `Error processing folder: ${relativeFolder}`, {
        error: result.error
      });
      errorCount++;
    }
  ...
  logger.info('CODEX_MD', 'CODEX.md generation complete', {
...
export async function generateCodexMd(dryRun: boolean): Promise<number> {
  const workingDir = process.cwd();
  const settings = SettingsDefaultsManager.loadFromFile(SETTINGS_PATH);
  const observationLimit = parseInt(settings.CODEX_MEM_CONTEXT_OBSERVATIONS, 10) || 50;

  logger.info('CODEX_MD', 'Starting CODEX.md generation', {
    workingDir,
    dryRun,
    observationLimit
  });
  ...
  if (trackedFolders.size === 0) {
    logger.info('CODEX_MD', 'No folders found in project');
    return 0;
  }

  logger.info('CODEX_MD', `Found ${trackedFolders.size} folders in project`);

  if (!existsSync(DB_PATH)) {
    logger.info('CODEX_MD', 'Database not found, no observations to process');
    return 0;
  }
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger.error('CODEX_MD', 'Fatal error during CODEX.md generation', {
      error: errorMessage
    });
    return 1;
  }
...
function processFilesForCleanup(
  ...
  for (const file of filesToProcess) {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.warn('CODEX_MD', `Error processing ${relativePath}`, { error: errorMessage });
      errorCount++;
    }
  }

  logger.info('CODEX_MD', 'CODEX.md cleanup complete', {
...
function cleanSingleFile(file: string, relativePath: string, dryRun: boolean): 'deleted' | 'cleaned' {
  ...
  if (stripped === '') {
    ...
    logger.debug('CODEX_MD', `${dryRun ? '[DRY-RUN] Would delete' : 'Deleted'} (empty): ${relativePath}`);
    return 'deleted';
  } else {
    if (!dryRun) {
      writeFileSync(file, stripped);
    }
    logger.debug('CODEX_MD', `${dryRun ? '[DRY-RUN] Would clean' : 'Cleaned'}: ${relativePath}`);
    return 'cleaned';
  }
}

export async function cleanCodexMd(dryRun: boolean): Promise<number> {
  const workingDir = process.cwd();

  logger.info('CODEX_MD', 'Starting CODEX.md cleanup', {
    workingDir,
    dryRun
  });
  ...
  function walkForCodexMd(dir: string): void {
    const ignorePatterns = [
      'node_modules', '.git', '.next', 'dist', 'build', '.cache',
      '__pycache__', '.venv', 'venv', '.idea', '.vscode', 'coverage',
      '.codex-mem', '.open-next', '.turbo'
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          if (!ignorePatterns.includes(entry.name)) {
            walkForCodexMd(fullPath);
          }
        } else if (entry.name === 'CODEX.md') {
          try {
  ...
  walkForCodexMd(workingDir);

  if (filesToProcess.length === 0) {
    logger.info('CODEX_MD', 'No CODEX.md files with auto-generated content found');
    return 0;
  }

  logger.info('CODEX_MD', `Found ${filesToProcess.length} CODEX.md files with auto-generated content`);

  try {
    return processFilesForCleanup(filesToProcess, workingDir, dryRun);
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger.error('CODEX_MD', 'Fatal error during CODEX.md cleanup', {
      error: errorMessage
    });
    return 1;
  }
