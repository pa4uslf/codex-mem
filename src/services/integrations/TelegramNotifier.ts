...
export async function notifyTelegram(input: TelegramNotifyInput): Promise<void> {
  ...
  if (settings.CODEX_MEM_TELEGRAM_ENABLED !== 'true') {
    return;
  }

  const botToken = settings.CODEX_MEM_TELEGRAM_BOT_TOKEN;
  const chatId = settings.CODEX_MEM_TELEGRAM_CHAT_ID;
  if (!botToken || !chatId) {
    return;
  }

  const triggerTypes = splitCsv(settings.CODEX_MEM_TELEGRAM_TRIGGER_TYPES);
  const triggerConcepts = splitCsv(settings.CODEX_MEM_TELEGRAM_TRIGGER_CONCEPTS);
