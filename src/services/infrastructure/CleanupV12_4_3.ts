...
export function runOneTimeV12_4_3Cleanup(
  ...
  if (process.env.CODEX_MEM_SKIP_CLEANUP_V12_4_3 === '1' && !dryRun) {
    logger.warn('SYSTEM', 'v12.4.3 cleanup skipped via CODEX_MEM_SKIP_CLEANUP_V12_4_3=1; marker not written');
    return;
  }
...
function executeCleanup(dbPath: string, effectiveDataDir: string, markerPath: string): void {
  ...
  backupPath = path.join(effectiveBackupsDir, `codex-mem-pre-12.4.3-${ts}.db`);
