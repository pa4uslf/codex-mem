...
export function getObservationQueueEngineName(): ObservationQueueEngineName {
  const raw = getQueueSetting('CODEX_MEM_QUEUE_ENGINE').trim().toLowerCase();
  if (raw === 'sqlite' || raw === 'bullmq') {
    return raw;
  }
  throw new Error(`Invalid CODEX_MEM_QUEUE_ENGINE=${raw}; expected sqlite or bullmq`);
}

export function getRedisQueueConfig(): RedisQueueConfig {
  const engine = getObservationQueueEngineName();
  const mode = normalizeRedisMode(getQueueSetting('CODEX_MEM_REDIS_MODE'));
  const url = getQueueSetting('CODEX_MEM_REDIS_URL').trim() || null;
  const host = getQueueSetting('CODEX_MEM_REDIS_HOST').trim() || '127.0.0.1';
  const port = parseRedisPort(getQueueSetting('CODEX_MEM_REDIS_PORT'));
  const prefix = sanitizePrefix(getQueueSetting('CODEX_MEM_QUEUE_REDIS_PREFIX'));
...
function normalizeRedisMode(value: string): RedisMode {
  ...
  throw new Error(`Invalid CODEX_MEM_REDIS_MODE=${value}; expected external, managed, or docker`);
}

function parseRedisPort(value: string): number {
  const port = Number.parseInt(value, 10);
  if (!Number.isInteger(port) || port <= 0 || port > 65535) {
    throw new Error(`Invalid CODEX_MEM_REDIS_PORT=${value}; expected a TCP port`);
  }
  return port;
}

function sanitizePrefix(value: string): string {
  return (value.trim() || 'codex_mem').replace(/[^a-zA-Z0-9_-]/g, '_');
}
...
function connectionFromUrl(rawUrl: string): RedisOptions {
  ...
  if (parsed.protocol !== 'redis:' && parsed.protocol !== 'rediss:') {
    throw new Error('CODEX_MEM_REDIS_URL must use redis:// or rediss://');
  }
  const db = parsed.pathname.length > 1
    ? Number.parseInt(parsed.pathname.slice(1), 10)
    : undefined;
  if (db !== undefined && (!Number.isInteger(db) || db < 0)) {
    throw new Error(`Invalid Redis database in CODEX_MEM_REDIS_URL: ${parsed.pathname}`);
  }
  return {
