...
### #646 - Plugin bricks Codex Code - stdin fstat EINVAL crash
- **Impact**: Plugin completely bricks Codex Code on Linux. Users cannot recover without manual config editing.
...
### #641/#609 - CODEX.md files in subdirectories
- **Impact**: CODEX.md files scattered throughout project directories
- **Note**: This is a documented feature request that was never implemented (setting exists but doesn't work)
- [ ] Implement the `disableSubdirectoryCODEXmd` setting properly
...
### #626 - HealthMonitor hardcodes ~/.codex path
- **Impact**: Fails for users with custom config directories
- [ ] Use configurable path instead of hardcoded ~/.codex
- [ ] Respect CODEX_CONFIG_DIR or similar env var
...
### #632 - Feature request for disabling CODEX.md in subdirectories
...
- Codex Code 2.1.1 compatibility + path validation (#614)
