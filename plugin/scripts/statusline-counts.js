...
const cwd = process.argv[2] || process.env.CODEX_CWD || process.cwd();
const project = basename(cwd);

try {
  let dataDir = process.env.CODEX_MEM_DATA_DIR || join(homedir(), ".codex-mem");
  if (!process.env.CODEX_MEM_DATA_DIR) {
    const settingsPath = join(dataDir, "settings.json");
    if (existsSync(settingsPath)) {
      try {
        const settings = JSON.parse(readFileSync(settingsPath, "utf-8"));
        if (settings.CODEX_MEM_DATA_DIR) dataDir = settings.CODEX_MEM_DATA_DIR;
      } catch { /* use default */ }
    }
  }
